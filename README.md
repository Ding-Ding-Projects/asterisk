...
## Recovered UI-smoke baseline

The recovered UI-smoke evidence baseline records 940 reviewed executable controls and a 17,127-row interaction contract. The committed parser derives 5,486 real action cases, multiplies them by the three language modes for 16,458 control rows, then adds 669 explicit route and equivalent proofs. It covers 29 explicitly inventoried paths: 10 preserved rescue records, 11 recovered source scripts, three producer-owned generated inventories, and five current documentation records replayed without replacing their newer content. The runtime path is limited to the cheap Lowlevel headless route. Raw runtime evidence remains outside the repository, and promotion is byte-preserving. This recovery establishes source and inventory evidence only. A new runtime capture is not yet verified.
