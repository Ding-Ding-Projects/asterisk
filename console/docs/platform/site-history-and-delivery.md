...
- The changelog viewer is generated from every local tag with a valid full object identifier, date, category, factual summary, and full commit URL. Its filtered view can be copied or exported to Markdown.
...
- The shared delivery rail exposes a persisted route tab strip with pinning. Static-host limitations are stated beside it: grouping and reordering are not offered by this route.
...
The module is `console/site/history-delivery.js`, loaded by the shared `console/site/app.js` registration on the six primary pages and directly by generated documentation pages. State uses the versioned `ding-pbx-site-history-delivery-v1` browser-storage key with a maximum of 250 events. Detail metadata is recursively restricted to a small allowlist and credentials, private vocabulary, paths, and file contents are omitted. `console/site/changelog-data.js` is generated from every local tag and `console/site/release-manifest.js` records the verified release state. The module does not make a runtime network request. `console/site/build.mjs` copies the module and `history.html` into deterministic published output, adds a real delivery mount host to generated article pages, and wires the module into those pages.
