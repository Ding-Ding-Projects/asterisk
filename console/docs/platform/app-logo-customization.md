...
**Desktop application:** Privileged logo inspection, conversion, decoder-status, cache read/asset-read/clear actions, desktop local-file picker, `LogoRuntime`, and the reachable `#surface=settings` route are wired. `sharp` runs in the packaged isolated worker, with CPU, Windows Job Object memory, monitored working-set, input/output, frame, pixel, target, aggregate, and full-pixel-reopen bounds enforced before cache replacement. The worker health status carries its revision, runtime version, native version, format capabilities, active JavaScript and native-file digests, and locked-package integrity. `npm run write:logo-manifest` fails when the native runtime set is absent and generates the exact packaged file list before a build can proceed. The checked-in manifest remains partial until that build-time proof runs. An OS-enforced AppContainer or restricted-token network capability boundary is still an open requirement, so no stronger network isolation claim is made.
