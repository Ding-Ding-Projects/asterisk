...
**The resolution derives the list of keys rather than restating it.** `supportStorageKeys()` returns the six constants this page actually writes — the settings record, the local history, the restricted-presentation record, the tickets themselves, the personal-vocabulary cache and the logo cache — and the resolution renders them beside the origin the browser reported, with a control that copies both. The two cache keys were inline string literals until this landed and are named constants now for exactly this reason: a hand-copied list here would be wrong the day a seventh key is added, and nothing would say so. This is the one place a locked-out reader is told what to clear, and the contract test re-derives the same set from every `localStorage` call in the file and refuses a mismatch in either direction — a key the page writes that the list omits, and a key on the list that nothing has ever written.
