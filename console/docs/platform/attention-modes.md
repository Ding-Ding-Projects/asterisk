...
The canonical notification producer inventory is `ATTENTION_SEVERITY_PRODUCERS` in `app/renderer/src/attention-modes.ts`. It enumerates 183 exact source call sites, helper names, columns, expected severity, and passive routing entries. `verifyAttentionSeverityProducers` source-scans both App and generated renderer files, rejects every unlisted producer, and requires warning or error for invalid input, unmet prerequisites, unavailable bridges, refused actions, and failed operations. Warning and error history uses `console.attention.noticeHistory`, schema version 1, a 200-entry bound, redacted text, durable restore, search, clear, and export. Redaction uses a bounded deterministic scanner for quoted and unquoted whitespace-containing Windows, UNC, POSIX, relative, PBX configuration, file URL, HTTP URL, and credential-shaped values. Corrupt or unsupported history stays untouched, is shown as unreadable, and is never silently treated as an empty history.
...
The canonical direct mutation inventory is `ATTENTION_MUTATION_INVENTORY` in `app/renderer/src/attention-modes.ts`. It records all 61 literal and generated `onUserMutation` call sites with exact source line and argument, occurrence, affected state, and clock effect. `verifyAttentionMutationInventory` source-scans both files and rejects every unlisted callback. Navigation, passive reads, selection, overlay state, and timers are explicit exclusions in `ATTENTION_MUTATION_PASSIVE_EXCLUSIONS`. The generated action/state tuples remain one record in `ATTENTION_MUTATION_ACTIONS` and are checked as complete tuples.
