...
- `package-squirrel.mjs` stops at the packaging-input preflight with
  `resources/school-mode-provenance.json` missing → the file is generated by
  `package-squirrel.mjs` itself from the release identity; nothing else writes it and it is
  git-ignored. If the preflight reports it, the write above the preflight was removed.
- electron-builder builds the setup executable, then fails with `Cannot detect repository by
