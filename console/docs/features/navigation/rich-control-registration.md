...
The central renderer exposes the registry to the compiled palette as `paletteItems` and exposes `richPaletteRows` with live readers, option providers, exact targets, and `executeRichControl` handlers. The checked-in design now renders those rich rows through the real M3 control component. Header activation selects the exact tab, scrolls and focuses the exact element, highlights it briefly, and reports a named stale-target result when the target is no longer present. Changing a registered value routes through the same generated control setter used by the visible editor, so persistence, validation, localization, and local history remain owned by the existing setting path. Action rows report that the action started, then report local-history acknowledgement or a history-unavailable state after the nested result is inspected. Rows without a canonical target are surfaced as explicit defects and are not silently rendered as destinations.
