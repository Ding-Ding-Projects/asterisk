...
The compiler currently reports 81 interactive loops carrying the declared `id` or `key` identity contract. Ordinary select, segmented, and chips producers emit option `id`, `key`, and `value` together. Order and pool producers emit stable `id` and `key` fields before the M3 Control receives them.
