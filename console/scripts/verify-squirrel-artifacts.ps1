[CmdletBinding()]
param(
  [string]$ArtifactDirectory = (Join-Path $PSScriptRoot '..\dist\squirrel-windows\squirrel-windows'),
  [Parameter(Mandatory = $true)][string]$ProvenancePath,
  [Parameter(Mandatory = $true)][string]$ExpectedCommit,
  [Parameter(Mandatory = $true)][string]$OutputPath,
  [string]$SetupFile = 'Material-Asterisk-Setup.exe',
  [string]$ExpectedPackageId = 'ding-pbx-console',
...
Require-Text $ExpectedVersion 'ExpectedVersion'
if ($RequiredPackageEntry.Count -eq 0) { Fail 'at least one RequiredPackageEntry is required' }
if ([IO.Path]::GetFileName($SetupFile) -ne $SetupFile) { Fail 'SetupFile must be a basename' }
$artifactRoot = (Resolve-Path -LiteralPath $ArtifactDirectory).Path
$provenanceFile = (Resolve-Path -LiteralPath $ProvenancePath).Path
$outputFile = [IO.Path]::GetFullPath($OutputPath)

try { $provenance = Get-Content -LiteralPath $provenanceFile -Raw | ConvertFrom-Json } catch { Fail 'provenance is not valid JSON' }
...
if ([string]$provenance.package.id -ne $ExpectedPackageId -or [string]$provenance.package.version -ne $ExpectedVersion -or [string]$provenance.package.architecture -ne $ExpectedArchitecture) { Fail 'provenance package identity/version/architecture does not match the expected candidate' }
...
if ($null -eq $signing.observedSignerInvocations -or @($signing.observedSignerInvocations).Count -ne 0) { Fail 'observedSignerInvocations must be an empty array' }
...
if (-not (Test-Path -LiteralPath $candidate -PathType Leaf)) { Fail "the expected setup file does not exist: $SetupFile" }
...
foreach ($line in $lines) {
  ...
  $records += [pscustomobject]@{ name = $fileName; size = $item.Length; sha1 = $sha1; sha256 = (Get-FileHash -LiteralPath $packagePath -Algorithm SHA256).Hash.ToLowerInvariant(); kind = if ($fileName -match '-delta\.nupkg$') { 'delta' } elseif ($fileName -match '-full\.nupkg$') { 'full' } else { 'other' } }
}

$packages = @(Get-ChildItem -LiteralPath $artifactRoot -Filter '*.nupkg' -File)
foreach ($package in $packages) { if (-not $indexed.Contains($package.Name)) { Fail "package is not indexed by RELEASES: $($package.Name)" } }
$fullCount = @($records | Where-Object kind -eq 'full').Count
$deltaCount = @($records | Where-Object kind -eq 'delta').Count
if ($fullCount -ne 1) { Fail "a clean candidate must contain exactly one full .nupkg, found $fullCount" }
...
$archive = [IO.Compression.ZipFile]::OpenRead((Join-Path $artifactRoot $fullPackage.name))
try {
  ...
  foreach ($required in $RequiredPackageEntry) {
    ...
    if (@($archive.Entries | Where-Object { $_.FullName -like $required }).Count -eq 0) { Fail "full package is missing required entry pattern: $required" }
  }
