...
const BREAKS = [
  ...
  {
    name: 'a reading-backed screen stops appending the shortfall to its own note',
    file: 'app/renderer/src/App.tsx',
    find: '    return [failed, this.droppedRowsNote(screen).trim()].filter(Boolean).join(\' \');',
  ...
  },
  {
    ...
    file: 'app/renderer/src/App.tsx',
    find: '    return [failed, this.droppedRowsNote(screen).trim()].filter(Boolean).join(\' \');',
    replace: '    return failed || this.droppedRowsNote(screen).trim();',
    tests: [WIRED],
  },
  {
  ...
  },
  {
    ...
    file: 'app/renderer/src/App.tsx',
    find: '      const value = valueOf(this.readings.ami?.managerUsers);\n      if (!value) return \'\';',
    replace: '      const value = valueOf(this.readings.ami?.managerUsers) ?? { users: [], total: 2 };',
    tests: [WIRED],
  },
  {
