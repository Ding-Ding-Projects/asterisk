...
const cases = [
  ...
  ['the key list quietly drops the key holding the lock itself',
    swap(APP, '    return [STORAGE_KEY,HISTORY_KEY,SCHOOL_KEY,SUPPORT_KEY,VOCABULARY_CACHE_KEY,LOGO_CACHE_KEY];',
      '    return [STORAGE_KEY,HISTORY_KEY,SUPPORT_KEY,VOCABULARY_CACHE_KEY,LOGO_CACHE_KEY];')],

  ['the key list names a key nothing has ever written',
    swap(APP, '    return [STORAGE_KEY,HISTORY_KEY,SCHOOL_KEY,SUPPORT_KEY,VOCABULARY_CACHE_KEY,LOGO_CACHE_KEY];',
      "    return [STORAGE_KEY,HISTORY_KEY,SCHOOL_KEY,SUPPORT_KEY,VOCABULARY_CACHE_KEY,LOGO_CACHE_KEY,'ding-pbx-pages-preferences'];")],

  ['the key list is hand-written back to literals, so it can drift from the constants',
    swap(APP, '    return [STORAGE_KEY,HISTORY_KEY,SCHOOL_KEY,SUPPORT_KEY,VOCABULARY_CACHE_KEY,LOGO_CACHE_KEY];',
      "    return ['ding-pbx-pages-v2','ding-pbx-pages-history-v1','ding-pbx-pages-school-v1','ding-pbx-pages-support-v1','ding-pbx-vocabulary-cache','ding-pbx-logo-cach'];")],
  ...
  ['the store is written unbounded, so one browser can hold an unbounded list',
    swap(APP, "    localStorage.setItem(SUPPORT_KEY,JSON.stringify({...supportStore,tickets:supportStore.tickets.slice(0,SUPPORT_LIMIT)}));",
      '    localStorage.setItem(SUPPORT_KEY,JSON.stringify(supportStore));')],
  ...
  ['the reset gate stops saying it leaves the tickets alone',
    swap(SETTINGS, ' It leaves the support tickets in this browser alone for the same reason: they are the recorded route back out of that lock.', '')],

  // ---- The records stop agreeing with the code ----

  ['the registry still calls the feature absent',
    swap(REGISTRY, '"support-tickets": {\n      "state": "implemented",', '"support-tickets": {\n      "state": "absent",')],
