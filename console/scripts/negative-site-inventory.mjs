...
const MATRIX_TEST = 'tests/contracts/site-registry-matrix-parity.test.mjs';
...
const cases = [
  ...
  [SCHEMA_TEST, 'a row names a path that does not exist',
    swap(REGISTRY, '"support-tickets": {\n      "status": "implemented-unverified",\n      "note": "The documentation site exposes a local Support Tickets desk from settings, its School-mode recovery disclosure, and documentation help. It records fictional tickets in browser storage, advances their status, and opens the actual browser storage guidance without sending data or deleting anything on the reader’s behalf. This remains implemented-unverified because no real browser interaction or capture ran in this lane.",\n      "implementation": {\n        "paths": [\n          "site/app.js",\n          "site/settings.html",\n          "site/documentation.html",', '"support-tickets": {\n      "status": "implemented-unverified",\n      "note": "The documentation site exposes a local Support Tickets desk from settings, its School-mode recovery disclosure, and documentation help. It records fictional tickets in browser storage, advances their status, and opens the actual browser storage guidance without sending data or deleting anything on the reader’s behalf. This remains implemented-unverified because no real browser interaction or capture ran in this lane.",\n      "implementation": {\n        "paths": [\n          "site/app.js",\n          "site/nowhere.js",\n          "site/documentation.html",')],
   ...
   * symbol now: `init` genuinely exists in site/app.js, so the validator's own
  ...
  [MATRIX_TEST, 'the generator status table drifts from the audited registry',
    swap(GENERATOR, "'support-tickets': 'implemented-unverified', 'unlock-ladder': 'absent'", "'support-tickets': 'absent', 'unlock-ladder': 'absent'")],
  ...
  [MATRIX_TEST, 'the generator status table can no longer be read back at all',
  ...
  // the state ollama.html is in, and the reason the check exists.
  [PAGES_TEST, 'an inert page stops being declared, so its dead controls go unrecorded',
    swap(PAGES_GUARD, "  ollama: 'ollama.html ships", "  ollamaX: 'ollama.html ships")],
  ...
  [PAGES_TEST, 'a declared-inert page is wired while the declaration stays',
    swap(APP, "  function applyLogo(", "  const wiredNow='ollama';\n  function applyLogo(")],
