...
export function validateAsteriskCatalog(catalog, inventory, files = new Set()) {
  ...
  if (!Array.isArray(catalog.modules) || !Array.isArray(catalog.resources)) throw new Error('Asterisk catalogue modules and resources arrays required');
  if (!Array.isArray(inventory?.sourceFamilies) || inventory.sourceFamilies.length !== 11) throw new Error('Asterisk catalogue family inventory must list all 11 source families');
  ...
  const ids = [...catalog.modules, ...catalog.resources].map((entry) => entry.id);
  if (new Set(ids).size !== ids.length) throw new Error('Asterisk catalogue identifiers must be unique');
  if (catalog.counts?.modules !== catalog.modules.length || catalog.counts?.resources !== catalog.resources.length) throw new Error('Asterisk catalogue counts drift');
  if (catalog.counts.modules !== inventory.expectedCounts?.modules || catalog.counts.resources !== inventory.expectedCounts?.resources) throw new Error('Asterisk catalogue hand-written expected counts drift');
  for (const entry of [...catalog.modules, ...catalog.resources]) {
  ...
  for (const path of [...(inventory.requiredGeneratedFiles ?? []), ...(inventory.requiredDocs ?? [])]) {
...
if (process.argv[1]?.replaceAll('\\', '/').endsWith('/verify-asterisk-catalog.mjs')) {
  try {
    ...
    const files = new Set([...inventory.requiredGeneratedFiles, ...inventory.requiredDocs].filter((path) => existsSync(resolve(root, path))));
