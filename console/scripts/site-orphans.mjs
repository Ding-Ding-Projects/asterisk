 ...
 *
 * This exists because of a specific, verified failure. The site once carried converter,
 * Ollama, and history pages plus helper scripts that the build and navigation did not
 * account for. The converter is now published and wired locally; the Ollama page is still
 * an honest unavailable surface and is linked from the published converter page. The
 * remaining recorded orphans are `site/history.html`, `full-builder.js`,
 * `history-delivery.js`, `release-manifest.js`, and `changelog-data.js`, which are
 * referenced by no published path the checker recognises.
 *
 * None of that failed anything. The site's own contract tests read a hard-coded list of
 * the six originally published pages, so an orphan page was invisible to every absence
 * claim they made: `local-file-converter.test.mjs` could assert that no converter surface
 * existed anywhere in the markup while a complete converter surface sat in the same
 * directory. A published article described the site converter's queue states in the
 * present tense.
 *
