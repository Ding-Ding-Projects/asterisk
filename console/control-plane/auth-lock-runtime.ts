...
export class FileAuthenticatorMetadataStore implements AuthenticatorMetadataStore {
  ...
  async reconcile(vault: AuthLockVault): Promise<void> { if (!vault.available) return; await this.#serialize(async () => { const value = await readJson<{ version: 1; entries: ReadonlyArray<import("../shared/authenticator.js").AuthenticatorEntryRecord>; pendingRemovals?: Array<{ id: string; credential: { vaultAccount: string; method: 'password' | 'totp' } } | string>; tombstones?: string[] }>(this.#path, { version: 1, entries: [], pendingRemovals: [], tombstones: [] }); const pending = (value.pendingRemovals ?? []).filter((item): item is { id: string; credential: { vaultAccount: string; method: 'password' | 'totp' } } => typeof item === 'object' && typeof item.id === 'string' && Boolean(item.credential)); const entries = [...value.entries]; for (const item of pending) { const entry = entries.find((candidate) => candidate.id === item.id); const credential = entry ? { vaultAccount: entry.credentialReference, method: item.credential.method } : item.credential; if (await vault.has(credential)) continue; const index = entries.findIndex((candidate) => candidate.id === item.id); if (index >= 0) entries.splice(index, 1); value.tombstones = [...(value.tombstones ?? []), item.id].slice(-10_000); } await writeJson(this.#path, { ...value, entries, pendingRemovals: pending.filter((item) => entries.some((entry) => entry.id === item.id)), tombstones: value.tombstones ?? [] }); }); }
}
