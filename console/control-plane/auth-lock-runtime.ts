...
export class FileAuthenticatorMetadataStore implements AuthenticatorMetadataStore {
  ...
  async reconcile(vault: AuthLockVault): Promise<void> { await this.#serialize(async () => { const value = await readJson<{ version: 1; entries: ReadonlyArray<import("../shared/authenticator.js").AuthenticatorEntryRecord>; pendingRemovals?: Array<{ id: string; credential: { vaultAccount: string; method: 'password' | 'totp' } } | string>; tombstones?: string[]; unresolvedRemovals?: string[] }>(this.#path, { version: 1, entries: [], pendingRemovals: [], tombstones: [], unresolvedRemovals: [] }); const entries = [...value.entries]; const pending: Array<{ id: string; credential: { vaultAccount: string; method: 'password' | 'totp' } }> = []; const unresolved = [...(value.unresolvedRemovals ?? [])]; for (const raw of value.pendingRemovals ?? []) { const id = typeof raw === 'string' ? raw : raw.id; const entry = entries.find((candidate) => candidate.id === id); const credential = entry ? { vaultAccount: entry.credentialReference, method: typeof raw === 'string' ? 'totp' as const : raw.credential.method } : typeof raw === 'string' ? undefined : raw.credential; if (!credential) { if (!unresolved.includes(id)) unresolved.push(id); continue; } if (!vault.available) { pending.push({ id, credential }); continue; } const present = await vault.has(credential); if (present && !(await vault.remove(credential))) { pending.push({ id, credential }); continue; } const index = entries.findIndex((candidate) => candidate.id === id); if (index >= 0) entries.splice(index, 1); value.tombstones = [...(value.tombstones ?? []), id].slice(-10_000); } await writeJson(this.#path, { ...value, entries, pendingRemovals: pending, tombstones: value.tombstones ?? [], unresolvedRemovals: unresolved }); }); }
}
