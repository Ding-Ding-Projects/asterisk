...
export class FileLockRecordPersistence implements LockRecordPersistence {
  ...
  async reconcileReceipt(vault: ToyLockCredentialVault): Promise<ToyLockReconciliationReceipt> { await this.reconcile(vault); return vault.available ? { status: 'reconciled', affectedIds: [] } : { status: 'pending-vault-unavailable', affectedIds: [], warning: 'Pending lock removals remain until the credential vault is available.' }; }
