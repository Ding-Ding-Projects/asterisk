...
$compilerKind = 'MSVC'
if ($compiler) {
  & $compiler.Source /nologo /std:c++17 /EHsc /O2 /DUNICODE /D_UNICODE $source $helperSource /Fe:$output /link /SUBSYSTEM:CONSOLE
} else {
  ...
  $compilerKind = 'MinGW'
  & $compiler.Source -std=c++17 -O2 $source $helperSource -o $output
}
