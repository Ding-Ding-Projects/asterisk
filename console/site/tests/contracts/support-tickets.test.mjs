...
function storageKeyLines() {
  const names = ['STORAGE_KEY', 'HISTORY_KEY', 'SCHOOL_KEY', 'VOCABULARY_CACHE_KEY', 'LOGO_CACHE_KEY'];
...
function loadDesk({
  ...
  const api = new Function(
    ...
    'setInterval', 'clearInterval', 'fetch', 'XMLHttpRequest', DESK_BODY,
  )(
    storage,
    { origin, hash },
...
test('supportStorageKeys names exactly the keys app.js actually writes, in both directions', () => {
   ...
   * it: every localStorage call in the file, resolved through its constant. A key added
   * to the file and not to the list fails; a key on the list that nothing writes fails
   * too, because a resolution naming a key that never existed sends somebody hunting. */
  const args = [...app.matchAll(/localStorage\.(?:getItem|setItem|removeItem)\(([^,)]+)/gu)].map((m) => m[1].trim());
  assert.ok(args.length >= 6, `only ${args.length} localStorage calls were found, which is too few to be reading the whole file`);
  const used = new Set();
