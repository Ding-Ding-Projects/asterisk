...
test('the COPY table defines a genuine four-level voice for both languages, on every key', () => {
  ...
  /* Then 16 -> 18 on 2026-08-26, when the Support Tickets desk landed with two keys:
...
test('the funny sliders reach only a small, explicitly wired subset of the six pages, not the pages themselves', () => {
   ...
   * wired to the sliders from the start like every card before it. Ten of eighteen keys
   * is still nowhere near the whole page, so the classification does not change. */
  assert.deepEqual([...wiredKeys].sort(),
    ['changelogDesc', 'dialogEmojisDesc', 'displayNameDesc', 'heroLede', 'motionDesc', 'narrationDesc',
      'schoolDesc', 'supportDesc', 'themeDesc', 'updatesDesc'],
