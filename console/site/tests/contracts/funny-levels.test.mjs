...
test('the COPY table defines a genuine four-level voice for both languages, on every key', () => {
  ...
  assert.equal(Object.keys(table).length, 10,
    `expected exactly 10 COPY keys, found ${Object.keys(table).length} -- update this pin if a message was deliberately added or removed`);
...
test('the funny sliders reach only a small, explicitly wired subset of the six pages, not the pages themselves', () => {
   ...
   * "implemented": the mechanism above is real, but only these three strings in the
   * whole site actually have a data-copy hook attached to them. */
  const html = pageText();
  const wiredKeys = new Set([...html.matchAll(/data-copy="(\w+)"/g)].map((m) => m[1]));
  assert.deepEqual([...wiredKeys].sort(), ['heroLede', 'motionDesc', 'themeDesc'],
