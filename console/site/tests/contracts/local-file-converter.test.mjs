...
test('every control the converter page ships is reached by name from app.js', () => {
  ...
  const ids = [...page.matchAll(/id="(converter-[a-z-]+)"/g)].map((match) => match[1]);
  assert.ok(ids.length >= 10, `only ${ids.length} converter control ids were found in the page -- this scan proves nothing`);
  ...
  assert.ok(controls.length >= 10, `only ${controls.length} of ${new Set(ids).size} converter ids are controls -- this scan proves nothing`);
...
test('the registry row now records the wired surface rather than the absence it used to', () => {
  ...
  for (const file of ['site/app.js', 'site/converter.html']) {
    assert.ok(row.files.includes(file), `the registry row does not name ${file}`);
  }
});
