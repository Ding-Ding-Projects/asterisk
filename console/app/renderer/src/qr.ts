...
export function qrSvg(valueText: string, accessibleLabel: string): string {
  ...
  if (bytes.length > 100) throw new Error('The pairing value is too long for the bundled QR encoder.');
  ...
  const rects: string[] = []; for (let r = 0; r < SIZE; r += 1) for (let c = 0; c < SIZE; c += 1) if (matrix[r]![c]) rects.push(`<rect x="${c}" y="${r}" width="1" height="1"/>`);
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="-4 -4 ${SIZE + 8} ${SIZE + 8}" role="img" aria-label="${accessibleLabel.replaceAll('&', '&amp;').replaceAll('"', '&quot;')}"><rect x="-4" y="-4" width="${SIZE + 8}" height="${SIZE + 8}" fill="white"/> <g fill="black">${rects.join('')}</g></svg>`;
}
