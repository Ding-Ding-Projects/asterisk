...
export function LockManagerSurface({ client, credentials, surfaceId, onNotice, onOpenSupportTickets, onOpenUnlockLadder }: LockManagerSurfaceProps) {
  ...
  useEffect(() => {
    void client.reconciliation?.().then(setReconciliation).catch(() => setReconciliation({ status: 'pending-vault-unavailable', affectedIds: [], warning: 'Toy-lock reconciliation is unavailable. Mutations remain blocked until the state can be read.' }));
  }, [client]);
  ...
  const remove = async (record: ToyLockRecord): Promise<boolean> => {
    ...
    if (reconciliation && reconciliation.status !== 'reconciled') { setError(reconciliation.warning); return false; }
    setBusy(true);
  ...
  return <section className="auth-surface" aria-labelledby="lock-manager-title">
    ...
    {removing ? <DestructiveActionGate actionLabel={`remove the lock for ${removing.targetId}`} onCancel={() => setRemoving(undefined)} onConfirm={async () => { if (await remove(removing)) setRemoving(undefined); }} /> : null}
