    ...
    return {
      ...
      ...(screen === 'history' ? (() => {
        ...
        return {
          ...
          backupRows: this.historyData.backups.filter((backup) => matches(`${backup.path} ${backup.createdAt} ${backup.status} ${backup.detail}`)).map((backup) => ({ label: `${this.selectedBackupPaths.has(backup.path) ? '☑' : '☐'} ${backup.createdAt || 'backup'} · ${backup.bytes} bytes · ${backup.status}`, color: backup.status === 'verified' ? '#9FF7C4' : '#FFD68A', pick: () => this.toggleBackupSelection(backup.path), verify: async () => { const response = await this.request('backup.retained.verify', { payload: { path: backup.path } }); this.toast(response?.message ?? backup.detail); } })),
          receiptRows: this.historyData.receipts.filter((receipt) => matches(`${receipt.id} ${receipt.action} ${receipt.remote} ${receipt.branch} ${receipt.status} ${receipt.detail}`)).map((receipt) => ({ label: `${receipt.action} · ${receipt.remote} · ${receipt.status}`, color: receipt.status === 'success' ? '#9FF7C4' : '#FFD68A', pick: () => this.toast(`${receipt.id}: ${receipt.detail}`) })),
