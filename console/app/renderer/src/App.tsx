...
What you can do: ${offered}.` : ''}`);
   ...
   */
  /** Two lanes independently wrapped `fire`: one to style copy by humour level and
   *  emoji setting, one to speak it. Composed here rather than picked between, and the
   *  order is the point -- the narrator speaks the STYLED text, so the humour level
   *  reaches speech exactly as the narration contract requires. Narrating the raw text
   *  would have the console say one thing while the screen showed another. */
  ...
  private syncWindowTitle(): void {
     ...
     * no Electron window at all, and a second capability added to the bridge by another
     * change is not automatically present in every caller's view of it -- guarding only
     * the bridge threw `window.setTitle is not a function` the first time the two met. */
