    ...
    h("div", { role: `group`, "aria-labelledby": v.labelId, "aria-describedby": v.describedBy, "aria-disabled": v.isDisabled, "data-control-kind": v.ctl.kind, "data-palette-control": v.ctl.presentationId ? true : false, "data-appearance-id": v.ctl.presentationId || ('control-' + v.ctl.id), style: sty(`display:flex; flex-direction:column; gap:10px; min-width:0; max-width:100%;`) },
      ...
      (v.isOrder ? h("div", { style: sty(`display:flex; flex-direction:column; gap:8px; min-width:0; max-width:100%;`) },
          h("ol", { "aria-labelledby": v.labelId, "aria-describedby": v.describedBy, style: sty(`list-style:none; margin:0; padding:0; display:flex; flex-direction:column; gap:6px; min-width:0;`) },
            A(v.dragItems).map(($i, $i$i) => R($i$i, h("li", { "data-appearance-id": "direct-m3control-0_0_25_1_1_1_1" + '-' + S(v.ctl?.presentationId ?? v.ctl?.id ?? '') + '-' + S($i?.id ?? $i?.key ?? $i?.label ?? $i$i), "data-direct-interactive": true, draggable: $i.draggable, "aria-current": $i.current, "aria-posinset": $i.position, "aria-setsize": v.orderSize, onDragStart: fn($i.onDragStart), onDragOver: fn($i.onDragOver), onDrop: fn($i.onDrop), onDragEnd: fn($i.onDragEnd), style: sty(`display:flex; align-items:center; gap:8px; min-height:48px; min-width:0; background:${S($i.bg)}; border:1px solid #333B34; border-radius:10px; padding:5px 6px 5px 12px; cursor:${S($i.cursor)};`) },
