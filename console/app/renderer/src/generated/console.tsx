...
const SCREENS = {
  ...
  customise:{ rail:'app', icon:'auto_awesome', label:'Customise everything', badge:'∞', title:'Customise everything', file:'console profile', kind:'generic',
    ...
    { title:'External editor', desc:'Hand a config file or an export to the editor you already use. The console works fully without one; this is a convenience, not something it needs.', ctls:[
      ctl('ed_status','Detection status','text','No supported editor has been detected yet.',{ action:'editor-status', info:'Detection runs in the privileged desktop process and reports normalized executable paths. An unavailable saved choice remains visible rather than being replaced silently.' }),
      ctl('ed_persistence','Saved editor settings','text','No saved editor settings have been written yet.',{ action:'editor-persistence', info:'The privileged editor store reports valid, missing, or invalid data. Invalid data stays visible with a reset action instead of being silently discarded.' }),
      ctl('ed_choice','Editor','select','',{ options:['Visual Studio Code','Visual Studio Code Insiders','Visual Studio Code Portable','Notepad++','Sublime Text','Notepad'], info:'Only editors actually installed on this machine are offered. A saved choice remains visible when it is unavailable, so the console never launches a different editor without your choice.' }),
      ctl('pbxadm:ed_custom_name','Other editor name','text','',{ placeholder:'My editor' }),
      ctl('pbxadm:ed_custom_path','Other editor executable','text','',{ placeholder:'C:\\tools\\editor.exe', info:'The program itself, not a command line. Browse for it rather than typing it if you are not sure; quotes and shell operators are refused because this launches a program directly rather than through a shell.' }),
      ctl('ed_custom_folder','Custom editor folder capability','select','Files only',{ options:['Files only','Folders as workspaces'], info:'Choose this only when the custom editor accepts a folder as a workspace root. The choice is explicit and stays attached to that custom record.' }),
      ctl('ed_browse','Browse for executable','segmented','Browse',{ options:['Browse'], action:'editor-browse' }),
      ctl('ed_pick_portable','Choose portable Visual Studio Code','segmented','Choose portable executable',{ options:['Choose portable executable'], action:'editor-pick-portable' }),
      ctl('ed_download','Open official download link','segmented','Open official download',{ options:['Open official download'], action:'editor-download' }),
      ctl('ed_download_vscode','Download Visual Studio Code','segmented','Download Visual Studio Code',{ options:['Download Visual Studio Code'], action:'editor-download-vscode' }),
      ctl('ed_save_custom','Save custom editor','segmented','Save',{ options:['Save'], action:'editor-save-custom' }),
      ctl('ed_remove_custom','Remove custom editor','segmented','Remove custom editor',{ options:['Remove custom editor'], action:'editor-remove-custom' }),
      ctl('ed_pick_project','Choose local project folder','segmented','Choose project folder',{ options:['Choose project folder'], action:'editor-pick-project' }),
      ctl('ed_project_status','Project folder state','text','Session only: no local project folder chosen.',{ action:'editor-project-status', info:'The chosen project folder is session-only and carries local-picker provenance. It is not persisted to the settings store. Reset it before choosing another folder.' }),
      ctl('ed_reset_project','Reset project folder','segmented','Reset project folder',{ options:['Reset project folder'], action:'editor-reset-project' }),
      ctl('ed_open_project','Open chosen project folder','segmented','Open project folder',{ options:['Open project folder'], action:'editor-open-project' }),
      ctl('ed_open_selected','Open selected file','segmented','Open selected file',{ options:['Open selected file'], action:'editor-open-selected' }),
      ctl('ed_open_vscode','Open in Visual Studio Code','segmented','Open in Visual Studio Code',{ options:['Open in Visual Studio Code'], action:'editor-open-vscode' }),
      ctl('ed_open_export','Open latest export in selected editor','segmented','Open latest export',{ options:['Open latest export'], action:'editor-open-export' }),
      ctl('ed_clear','Forget the chosen editor','segmented','Forget chosen editor',{ options:['Forget chosen editor'], action:'editor-clear-choice' }),
      ctl('ed_reset_storage','Reset invalid editor settings','segmented','Reset editor settings',{ options:['Reset editor settings'], action:'editor-reset-storage' }),
      ctl('ed_cancel_operation','Cancel active editor operation','segmented','Cancel active operation',{ options:['Cancel active operation'], action:'editor-cancel-operation' })
    ]},
    ...
    return {
      ...
      histActions:[
        ...
        { icon:'search', label:'Search history', run:() => this.setState({ regexOpen:true, regexTarget:'nav', regexX:'40%', regexY:'160px' }) },
          { icon:'download', label:'Export bundle', run:() => { if (this.exportUnavailable) this.exportUnavailable('History export unavailable', 'The history export needs the local history store to provide its real commits. No placeholder bundle was created.'); else this.toast('History export unavailable'); } }
      ],
      ...
      subItems:(() => {
        ...
        if (s.ctxSub === 'gsave') return [
          { icon:'download', label:'Export group as JSON', run:() => { close(); const content = JSON.stringify({ version:1, name:g.name, colour:g.colour, collapsed:!!g.collapsed, tabs:g.tabs }, null, 2); if (this.onExportRecord) this.onExportRecord({ name:'tab-group.json', content, source:'tab-group' }); else this.fire('Group export unavailable', 'The desktop export bridge is not mounted.'); } },
        ...
        if (s.ctxSub === 'tabexport') { const tabKey = s.ctxTabKey || s.screen; return [
          { icon:'download', label:'Export this tab', run:() => { close(); const content = JSON.stringify({ version:1, tab:tabKey, label:s.tabNames[tabKey] || (SCREENS[tabKey] ? SCREENS[tabKey].title : tabKey), pinned:s.pinned.indexOf(tabKey) >= 0, colour:s.tabColours[tabKey] || null }, null, 2); if (this.onExportRecord) this.onExportRecord({ name:'tab.json', content, source:'tab' }); else this.fire('Tab export unavailable', 'The desktop export bridge is not mounted.'); } },
          { icon:'download_for_offline', label:'Export all tabs', run:() => { close(); const content = JSON.stringify({ version:1, tabs:s.tabs, pinned:s.pinned, groups:s.groups }, null, 2); if (this.onExportRecord) this.onExportRecord({ name:'tabs.json', content, source:'tabs' }); else this.fire('Tab export unavailable', 'The desktop export bridge is not mounted.'); } },
      ...
      ctxItems:(() => {
        ...
        if (s.ctxKind === 'row') {
          ...
          return decorate([
            ...
            { icon:'download', label:'Export as configuration', hint:'', run:() => { close(); if (this.exportUnavailable) this.exportUnavailable('Configuration export unavailable', name + ' has no completed local read-back on this screen, so no guessed file was created.'); else this.toast('Configuration export unavailable'); } },
      ...
      appearActions:[
        ...
        { icon:'download', label:'Export', run:() => { if (this.exportUnavailable) this.exportUnavailable('Appearance export unavailable', 'The appearance export needs the mounted renderer serializer. No placeholder file was created.'); else this.toast('Appearance export unavailable'); } },
