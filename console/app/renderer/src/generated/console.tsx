...
const SCREENS = {
  ...
  security:{ rail:'sys', icon:'shield', label:'Security', badge:'!', title:'Security', file:'acl.conf', kind:'table',
    sub:'Named access control lists, listed in the exact order Asterisk evaluates them -- the LAST matching rule wins, which is what makes a broad deny followed by a narrow permit work as an allowlist. Certificates are chosen from the machine store — no path is ever typed.',
    ...
    return {
      ...
      ctxItems:(() => {
        ...
        if (s.ctxKind === 'row') {
          ...
          return decorate([
            { icon:'edit', label:'Edit ' + name + '…', hint:'↵', run:() => { close(); (isAclRow && this.onPickRow) ? this.onPickRow(name) : this.setState({ wizardOpen:true, wizardStep:0 }); } },
            ...
            { icon:'delete', label:'Delete ' + name, hint:'⌦', run:() => { close(); this.areYouSure('Delete ' + name, sc.kind === 'servers' ? 'This connection profile is removed from the console. It does not touch or reconfigure the target machine.' : (isAclRow ? 'This rule is removed from acl.conf on the target -- backed up first, and rolled back automatically if the write cannot be verified.' : 'This object and everything referencing it are removed. The four gates still apply after the minigame.'), 3, () => (sc.kind === 'servers' && this.onRemoveServerRow ? this.onRemoveServerRow(name) : (isAclRow && this.onRemoveAclRule ? this.onRemoveAclRule(name) : this.ceremony('Delete ' + name, 'delete ' + name)))); } },
            common[0], common[1]
          ]);
        }
        if (s.ctxKind === 'search') {
