...
const SCREENS = {
  ...
  customise:{ rail:'app', icon:'auto_awesome', label:'Customise everything', badge:'∞', title:'Customise everything', file:'console profile', kind:'generic',
    ...
    { title:'Spoken narration', desc:'The console can read what happens aloud. Off until you switch it on, and it never speaks over a screen reader -- it yields rather than competing.', ctls:[
      ctl('nar_enabled','Speak what happens','switch',false,{ info:'Narration is infrequent by design: one line at a time, debounced, and a superseded line replaces the queued one rather than stacking behind it. An error is always spoken and names the actual failure, whatever the funny level.' }),
      ctl('nar_language','Spoken language','segmented','en',{ options:['en','zh','both'], info:'Both speaks English then Cantonese, strictly one after the other, never overlapping.' }),
      ctl('nar_voice_en','English voice','select','Choose automatically',{ options:['Choose automatically'], info:'Only voices actually installed on this computer are listed, read from the platform at launch rather than from a fixed list. Choose automatically is the default because no install can know what you have.' }),
      ctl('nar_voice_zh','廣東話 voice','select','Choose automatically',{ options:['Choose automatically'], info:'Its own picker, because choosing an English voice says nothing about which Cantonese voice should read the other half of a bilingual line.' }),
      ctl('nar_rate','Speaking rate','slider',1,{ min:0.5, max:2, step:0.1, info:'The difference between a narrator you keep on and one you endure.' }),
      ctl('nar_pitch','Pitch','slider',1,{ min:0, max:2, step:0.1 }),
      ctl('nar_status','Voice status','text','Not started.',{ action:'narration-status', info:'Says what will actually speak: whether a chosen voice is installed, whether it is network-backed and will go quiet offline, or whether nothing on this machine can read the language at all. A picker showing a value implies that value will be heard, which is exactly what needs saying out loud when it will not.' })
    ]},
    ...
    { title:'Language', desc:'Which language the console speaks. Bilingual keeps English primary and adds the Cantonese beside it. Technical identifiers -- codecs, config keys, section names, SIP URIs -- stay literal in every mode, because they have to survive being read back and typed.', ctls:[
      ...
      ctl('fun_english','English funny level','slider',5,{ min:1, max:5, info:'Level 1 is fully serious. Level 5 is maximum playfulness. This only styles English voice; it never changes facts or technical values.' }),
      ctl('fun_cantonese','Cantonese funny level','slider',5,{ min:1, max:5, info:'Level 1 is fully serious. Level 5 is maximum playfulness. This only styles Hong Kong Cantonese voice; it never changes facts or technical values.' }),
