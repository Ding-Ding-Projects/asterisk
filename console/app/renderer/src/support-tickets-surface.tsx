...
import { SUPPORT_TICKET_CATEGORIES, SUPPORT_TICKET_DISCLOSURE, SUPPORT_TICKET_SEVERITIES, type SupportTicket } from '../../../shared/support-tickets';
...
export function SupportTicketsSurface({ client, applicationDataPath, openApplicationDataFolder, onNotice }: SupportTicketsSurfaceProps) {
  ...
  const [severity, setSeverity] = useState<string>(SUPPORT_TICKET_SEVERITIES[1]);
  ...
  return <section className="auth-surface" aria-labelledby="support-tickets-title">
    ...
    <div className="auth-disclosure" role="note">{SUPPORT_TICKET_DISCLOSURE} The resolution only opens a folder. It never deletes anything.</div>
    {error ? <div className="auth-error" role="alert">{error}</div> : null}
    <div className="auth-grid"><form className="auth-card" onSubmit={(event) => void create(event)}><h3>Open a local ticket</h3><label>Category<select value={category} onChange={(event) => setCategory(event.target.value)}>{SUPPORT_TICKET_CATEGORIES.map((item) => <option key={item}>{item}</option>)}</select></label><label>Description<textarea value={description} onChange={(event) => setDescription(event.target.value)} rows={5} maxLength={2000} placeholder="Describe what the local desk should show you next." required /></label><label>Severity<select value={severity} onChange={(event) => setSeverity(event.target.value)}>{SUPPORT_TICKET_SEVERITIES.map((item) => <option key={item}>{item}</option>)}</select></label><button className="auth-button" type="submit" disabled={busy}>Create local ticket</button></form><div className="auth-card"><h3>Resolution path</h3><p className="auth-help">A lockout is resolved by deleting the app's local application-data folder yourself. This desk only opens it in the platform file manager.</p><dl className="auth-facts"><div><dt>Exact folder</dt><dd><code>{applicationDataPath}</code></dd></div><div><dt>Deletion</dt><dd>Never performed by this app</dd></div></dl><button className="auth-button secondary" type="button" onClick={() => void openFolder()} disabled={busy}>Open folder in file manager</button></div></div>
    <div className="auth-list-card"><div className="auth-list-toolbar"><div><h3>Local ticket list</h3><p>{visible.length} visible, {tickets.length} total</p></div><div className="auth-toolbar-controls"><input aria-label="Search support tickets" placeholder="Search ticket number, category or status" value={query} onChange={(event) => setQuery(event.target.value)} /></div></div>{visible.length === 0 ? <p className="auth-empty">No local tickets match this search.</p> : <div className="auth-entry-list">{visible.map((ticket) => <article className="auth-entry" key={ticket.id}><div className="auth-entry-heading"><div><h4>{ticket.id} · {ticket.category}</h4><p>{ticket.description}</p></div><span className="auth-status">{ticket.status}</span></div><p>Severity: {ticket.severity} · {new Date(ticket.openedAt).toLocaleString()}</p><p className="auth-help">{ticket.firstResponse}</p>{ticket.status !== 'Resolved' ? <button className="auth-button secondary" type="button" onClick={() => void advance(ticket)} disabled={busy}>Advance local response</button> : <p className="auth-help">Resolution ready: open the exact folder above. The ticket still deletes nothing.</p>}</article>)}</div>}</div>
  </section>;
}
