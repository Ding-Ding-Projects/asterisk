...
export const ATTENTION_WIRING: readonly AttentionWiringRow[] = [
  ...
  /* The sixth row is not a mode: it is the next-action VALUE that "One thing at a time"
   * exists to keep visible. It used to be a design control, `ctl('att_next',...)`, edited
   * on a settings screen, and every marker here named that surface. The value is still
   * carried, persisted under the same key and read back, but it is now typed at the
   * attention rail's own input beside the work rather than in a settings list -- which is
   * what the design's own promise for it asks for ("remains visible wherever work is
   * shown"), and which `tests/contracts/attention-modes.test.mjs` pins from the other
   * side by requiring the design's attention group to declare exactly the five switches.
   * Re-anchored to the surface that ships rather than restoring one that two contracts
   * now agree should not exist. The row stays fail-closed: each marker below still has to
   * match exactly once, and `verify-inventories.mjs` deletes each one in turn to watch
   * this check go red. */
  { id:'next-action', control:'attn_rail_next_input', mode:null, designMarker:marker('design',"Keeps one next action visible, chosen by you. It survives a context switch."), controlConstruction:marker('app',"className: 'attn-rail-next-input',"), durableKey:marker('module',"nextAction: 'console.attention.nextAction'"), writerMarkers:[marker('app',"'aria-label': 'The one thing you are doing right now',"), marker('module',"export function setNextAction(")], setterMarkers:[marker('app',"setNextAction(storage, event.target.value);"), marker('module',"if (trimmed) storage.setItem(NEXT_ACTION_SETTING_KEY, trimmed);")], consumerMarkers:[marker('app',"value: nextAction(storage),"), marker('module',"export function nextAction(")] }
];
