...
export const ATTENTION_WIRING: readonly AttentionWiringRow[] = [
  ...
  /* Re-derived on 2026-08-27, because this row described a control that does not exist.
   *
   * It named ten markers and EIGHT of them matched nothing: a design control
   * `ctl('att_next','Current next action','text','')`, a settings-panel construction
   * `'att_next': 'nextAction'`, a setter branch keyed on `control?.id === 'att_next'`, and a
   * consumer writing `action.textContent = 'Next action: ...'`. None of it is in the tree.
   * The design declares five attention controls and no sixth text field; the console builds
   * the next-action field as an input on the attention rail instead, so the row was checking
   * a settings control that was specified and then implemented somewhere else entirely.
   *
   * It went unnoticed because `verify-inventories.mjs` was already exiting on the site
   * registry's schema before it ever reached this check -- one red hiding another.
   *
   * The markers below are the chain that is really there, end to end: the design's own
   * promise of the behaviour (made on the One-thing control, which is where the design puts
   * it), the rail input that realises it, the durable key, the bounded setter, and the
   * reader the input's value comes from. Every one was measured to occur exactly once in its
   * owner before being written here, rather than being asserted and hoped for. */
  { id:'next-action', control:'att_next', mode:null, designMarker:marker('design',"Keeps one next action visible, chosen by you."), controlConstruction:marker('app',"className: 'attn-rail-next-input',"), durableKey:marker('module',"nextAction: 'console.attention.nextAction'"), writerMarkers:[marker('app',"setNextAction(storage, event.target.value);")], setterMarkers:[marker('module',"export function setNextAction(storage: ModeStorage, value: string): void {"), marker('module',"const trimmed = value.trim().slice(0, NEXT_ACTION_MAX_LENGTH);")], consumerMarkers:[marker('app',"value: nextAction(storage),"), marker('app',"'aria-label': 'The one thing you are doing right now',"), marker('module',"export function nextAction(storage: ModeStorage | undefined): string {"), marker('module',"return typeof value === 'string' ? value.slice(0, NEXT_ACTION_MAX_LENGTH) : '';")] }
];
