...
export const ATTENTION_WIRING: readonly AttentionWiringRow[] = [
  ...
  /* The next action is the one row here that is not a settings switch, and this row used to
   * describe it as if it were: a design control `att_next`, an `App.ATTENTION_CONTROLS` entry
   * beside the five modes, an `onUserMutation` writer, and a consumer writing `Next action: ...`
   * into an element's textContent. Not one of those eight seams has ever existed in any of the
   * four sources this inventory reads, in any commit -- so the row asserted a shape of wiring
   * that was only ever proposed, while the shipped implementation sat beside it unnamed.
   *
   * What actually ships is the attention rail's own text input, built in `attentionOverlay()`
   * and covered by tests/ui/attention-modes-wired.test.tsx. That is what the canonical
   * requirement asks for: the chosen action visible where the work happens rather than parked
   * on a settings page. The design does not draw the field at all; what the design contributes
   * is the sentence under the `att_one` switch stating the requirement, so that sentence is
   * this row's design marker rather than a control that is not there.
   *
   * The markers are deliberately NOT the rail's rendering. `one-thing` above already names all
   * four of those seams -- `showNextAction`, the `attn-rail-next` div, `value: nextAction(...)`
   * and the `setNextAction(...)` call site -- because they are what its switch turns on. A first
   * attempt at this repair reused them and produced a row that was a copy of its neighbour:
   * removing any of the four failed on `one-thing` first, so the sixth row was watching nothing
   * of its own. What is left for this row is the value's own storage seam, which no other row
   * touches: the control that carries it, its durable key, its bounded writer, its reader, and
   * the clamp that bounds what a hand-edited store can put on screen. */
  { id:'next-action', control:'attn-rail-next-input', mode:null, designMarker:marker('design',"Keeps one next action visible, chosen by you. It survives a context switch."), controlConstruction:marker('app',"className: 'attn-rail-next-input'"), durableKey:marker('module',"nextAction: 'console.attention.nextAction'"), writerMarkers:[marker('module',"if (trimmed) storage.setItem(NEXT_ACTION_SETTING_KEY, trimmed);"), marker('module',"else if (storage.removeItem) storage.removeItem(NEXT_ACTION_SETTING_KEY);")], setterMarkers:[marker('module',"export function setNextAction(storage: ModeStorage, value: string): void {")], consumerMarkers:[marker('module',"export function nextAction(storage: ModeStorage | undefined): string {"), marker('module',"return typeof value === 'string' ? value.slice(0, NEXT_ACTION_MAX_LENGTH) : '';"), marker('app',"'The one thing you are doing right now'")] }
];
