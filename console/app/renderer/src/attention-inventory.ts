...
export const ATTENTION_WIRING: readonly AttentionWiringRow[] = [
  ...
  /* The sixth control is not a mode -- it is the one chosen next action itself, which is why
   * verifyAttentionWiring expects ATTENTION_MODES.length + 1 rows rather than one per mode.
   *
   * Every marker in this row used to name a settings-page text control that was planned and
   * never built: a design `ctl('att_next','Current next action','text','')`, an entry in
   * App's ATTENTION_CONTROLS map, a string branch in setVal, and a consumer assigning
   * `action.textContent`. Not one of those strings has ever existed anywhere in this
   * repository. The feature shipped in a different shape -- an input in the attention rail,
   * above every screen -- and App.tsx says why in its own words: rendered there "rather than
   * as a row on a settings page, so 'where the work happens' means the console itself and not
   * a place you have to go looking". The row was left pinned to the abandoned shape, so the
   * whole inventory gate refused at this line while the feature underneath it worked.
   *
   * Retargeted at what exists, rather than deleted. Deleting it would have taken the sixth
   * control out of a count that is correct, and would have left the field's accessible name,
   * its placeholder, its 140-character bound and its clear-on-empty behaviour guarded by
   * nothing at all.
   *
   * `designMarker` carries an app-owned marker here, and that is the honest reading rather
   * than a loophole: this control is genuinely not in the design. It is one of the surfaces
   * App adds above the compiled shell, like the command palette and the title-bar name. The
   * five rows above it are real design controls and still say so.
   *
   * Deliberately disjoint from the `one-thing` row, which already pins the rail container,
   * `value: nextAction(storage)` and the `setNextAction(storage, event.target.value)` call.
   * This row pins what that one does not: how the field announces itself, and what the module
   * does with the value on the way in and out. Two rows guarding one string each would be one
   * row wearing two hats. */
  { id:'next-action', control:'att_next', mode:null, designMarker:marker('app',"className: 'attn-rail-next-input',"), controlConstruction:marker('app',"placeholder: 'Choose the one thing you are doing',"), durableKey:marker('module',"nextAction: 'console.attention.nextAction',"), writerMarkers:[marker('module','export function setNextAction(storage: ModeStorage, value: string): void {'), marker('module','const trimmed = value.trim().slice(0, NEXT_ACTION_MAX_LENGTH);')], setterMarkers:[marker('module','if (trimmed) storage.setItem(NEXT_ACTION_SETTING_KEY, trimmed);'), marker('module','else if (storage.removeItem) storage.removeItem(NEXT_ACTION_SETTING_KEY);'), marker('module','export const NEXT_ACTION_MAX_LENGTH = 140;')], consumerMarkers:[marker('module','export function nextAction(storage: ModeStorage | undefined): string {'), marker('app',"'aria-label': 'The one thing you are doing right now',")] },
];
