...
test('the shell now declares roles and accessible names, so the readers below are a choice not a necessity', () => {
  ...
  /* Moved 2026-08-27, upward, and here is the why this message asks for.
   *
   * Roles: 32 -> 33. The Tab search overlay is built by `scripts/extend-pbx-m3.mjs`, not by
   * the design, and the raw design compile has only 32. That overlay's own `role: dialog`
   * was always there; what changed is that the pin was taken against a compile the
   * extension had not run over. It is now taken against the shipped shell, which is the
   * file the harness below actually reads.
   *
   * Accessible-name attributes: 26 -> 28. Two of that same overlay's attributes: its
   * `aria-label`, for the same reason as above, and one genuinely new `aria-modal` added in
   * this pass -- see the dialog test below for what it fixes. */
  const ROLE = 'role' + ':';
  const ARIA = 'aria' + '-';
...
test('every modal surface now carries the dialog role, not just the palette card', () => {
   ...
   *
   * The fifteenth is the Tab search overlay, and it is worth recording what the second
   * assertion below caught on 2026-08-27. That overlay had `role: dialog` and no
   * `aria-modal`, against a scrim it paints at `inset: 0`, z-index 82, that closes it on
   * click -- so a pointer could reach nothing behind it while a screen reader was free to
   * walk straight out into that unreachable content. The pin read 14 against 15 and went
   * red, which is exactly the job. Fixed in `scripts/extend-pbx-m3.mjs`, which is where
   * that overlay is built; the generated shell is never hand-edited. */
