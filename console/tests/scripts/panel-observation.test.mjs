...
test('the shell now declares roles and accessible names, so the readers below are a choice not a necessity', () => {
  ...
  /* Which way and why, 2026-08-27. Roles 32 to 33 and accessible-name attributes 26 to 28.
   *
   * Roles: +1, the Tab search overlay, which arrived with the console feature integration
   * carrying `role: dialog` and an `aria-label`. Nothing re-derived these pins in that
   * commit, so both had been failing on `master` rather than guarding anything.
   *
   * Accessible names: +1 for that same overlay's `aria-label`, and +1 more from this pass,
   * because the overlay declared `role: dialog` and NOT `aria-modal` while painting a
   * full-inset scrim that closes it on click. That is a real defect rather than drift -- a
   * screen reader was free to walk into content the pointer could not reach -- and it is
   * fixed in `scripts/extend-pbx-m3.mjs` and recompiled, not patched into the generated
   * file. The dialog test below is what named it: it asserts modal === dialogs, and 14
   * against 15 is the shape that says exactly one dialog is lying about being modal.
   *
   * Composition, so the next reader can check rather than trust: banner 1, button 6,
   * tablist 1, tab 2, navigation 2, main 1, tabpanel 1, dialog 15, menu 2, menuitem 2 --
   * 33, with no `role:` occurrence outside a template literal. Accessible names:
   * aria-expanded 1, aria-label 7, aria-selected 2, aria-controls 2, aria-labelledby 1,
   * aria-modal 15 -- 28. */
  const ROLE = 'role' + ':';
  const ARIA = 'aria' + '-';
...
test('every modal surface now carries the dialog role, not just the palette card', () => {
   ...
   *
   * 14 to 15 on 2026-08-27, and the second assertion below is what earned its keep. The
   * fifteenth is the Tab search overlay, which the console feature integration added with
   * `role: dialog` and no `aria-modal` -- so the count read 15 against 14 and named the one
   * dialog that was lying about being modal, rather than leaving it to be noticed by
   * somebody using a screen reader. Fixed at the source in `scripts/extend-pbx-m3.mjs` and
   * recompiled; the generated shell is never hand-edited. */
