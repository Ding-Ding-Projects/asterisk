...
if "%SILENT_MODE%"=="0" (
  ...
  if errorlevel 1 (
    echo [build] Requesting elevation before any build work begins.
...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_ROOT%console\scripts\build-console.ps1" -SkipBootstrap %BUILD_SILENT%
exit /b %ERRORLEVEL%
