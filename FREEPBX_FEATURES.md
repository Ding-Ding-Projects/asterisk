...
- **PJSIP device fields (CORE-PJSIP-DEV-*):** Creation verified end to end; editing still unproven. `console/control-plane/subsystem-models.ts` carries a typed model treating the endpoint, auth and aor trio as one identity, and `console/app/renderer/src/endpoint-create.ts` builds that trio from the guided wizard’s own answers and applies it through the existing plan-and-apply path. Verified on 2026-08-23 by running those exact functions against the live target: the endpoint was written, `pjsip reload` run, and Asterisk’s own `pjsip show endpoint 7100` reported the endpoint and its `InAuth` object; the target was then restored to `No objects found`. Eleven local regressions cover the positive path and the refusals — a missing name, an illegal section name, a duplicate, and a secret leaking into the confirmation plan. Two defects were found writing them: the wizard previously finished by running `pjsip reload` against a file it had never written to and reported success, and the “behind a home router” question applied its three settings only when the answer was yes, while the model already set all three, so answering no changed nothing. Still not earning a tick: the bound `e_*` controls edit an existing endpoint but that path has not been run against a target, and there is no list, edit or delete destination — which is what these items specify.
