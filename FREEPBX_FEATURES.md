...
- **PJSIP device fields (CORE-PJSIP-DEV-*):** Existing — read-only for the subset exposed by `pjsip show endpoints`/`show contacts`/`show registrations`/`show auths`/`show transports` (state, contact URI, registration status only — no endpoint/AOR/auth object field editing). Verified against `configs/samples/pjsip.conf.sample`: every field this subsection names (`secret`, `dtmf_mode`, `context`, `trust_id_inbound`/`send_pai`, `qualify_frequency`, `transport`, `rtp_bind_udp`/`ice_support`/`use_avpf`/`rtcp_mux`, `bundle`, `max_audio_streams`/`max_video_streams`, `allow`/`disallow`, `callgroup`/`pickupgroup`, custom dial via `Contact`, `mailbox`/`voicemail_extension`, `max_contacts`/`remove_existing`, `use_received_transport`, `rtp_symmetric`/`rewrite_contact`/`force_rport`, `mwi_subscribe_replaces_unsolicited`/`aggregate_mwi`, `media_encryption`, `media_encryption_optimistic`, `timers`/`timers_min_se`, `direct_media`/`media_address`, `refer_blind_progress`, `device_state_busy_at`, `identify_by`, `min_expiry`/`max_expiry`, `rtp_timeout`/`rtp_timeout_hold`, `outbound_proxy`/`outbound_auth`, `message_context`) is a real key documented in that sample file. No editor renders, validates, applies, or reads any of them back today, so nothing in this subsection clears the adoption bar; there is also no `pjsip_wizard.conf`/`pjsip_notify.conf` field model despite both files being on the writable allowlist (lines 108-109).
