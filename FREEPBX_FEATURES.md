...
- **PJSIP device fields (CORE-PJSIP-DEV-*):** Backend ready, no UI. `console/control-plane/subsystem-models.ts` now carries a real `parsePjsip`/`validatePjsip`/`toConfigValuePjsip`/`newPjsipEndpoint` model (2026-08-23) covering the endpoint/auth/aor trio as one atomic identity, with every field cited to its exact line in `configs/samples/pjsip.conf.sample` and locally verified with 533/533 control-plane tests (positive samples, every validator's negative case, a byte-identical no-change round trip, in-place field edits, trio deletion, and a brand-new endpoint applying cleanly with a real random secret). Applied and read back against the live target: created endpoint `7000` (context, disallow/allow, dtmf_mode, direct_media, force_rport, rewrite_contact, rtp_symmetric, auth, aors, plus a digest auth object and a max_contacts=1/remove_existing=yes AoR), reloaded PJSIP, and confirmed every one of those fields in `pjsip show endpoint 7000`'s live parameter dump before restoring the target's original all-comments sample. During this pass, six keys the earlier audit assumed were present (`bundle`, `callgroup`, `pickupgroup`, `use_received_transport`, `min_expiry`/`max_expiry`, `message_context`, `rtcp_mux`) were found to have **no** documented `;key=value` line anywhere in `pjsip.conf.sample` — only passing mentions — and are correctly absent from the new model; that overstatement in this document is now corrected. No item below is checked because **no destination renders these fields as controls yet** — the model exists only in the control plane, with no PBX-rail editing surface, no interactive `select`/`stepper`/`switch` controls, and no `showWhen` conditional wiring, so the adoption bar's "rendered as a real control" requirement is unmet for all of them. `dtmf_mode`'s non-default enum members (`inband`/`info`/`auto`/`auto_info`) are the standard res_pjsip.so set but are not directly sample-verified beyond the documented default of `rfc4733`. There is also still no `pjsip_wizard.conf`/`pjsip_notify.conf` field model despite both files being on the writable allowlist (lines 108-109).
...
- [ ] CORE-PJSIP-DEV-001 — Secret/password.
- [ ] CORE-PJSIP-DEV-002 — Account Code and Default User.
- [ ] CORE-PJSIP-DEV-003 — DTMF mode, including version-appropriate RFC 4733 behavior.
- [ ] CORE-PJSIP-DEV-004 — Context.
...
- [ ] CORE-PJSIP-DEV-007 — Active Transport.
...
- [ ] CORE-PJSIP-DEV-017 — RTP Symmetric, Rewrite Contact, and Force rport.
- [ ] CORE-PJSIP-DEV-018 — MWI Subscription Type and Aggregate MWI.
- [ ] CORE-PJSIP-DEV-019 — Media Encryption: none, SDES, or DTLS where supported.
...
- [ ] CORE-IAX-DEV-001 — Secret, Context, Host, and friend/peer/user Type.
- [ ] CORE-IAX-DEV-002 — Transfer mode: yes, no, or media-only.
- [ ] CORE-IAX-DEV-003 — Port, Qualify, Disallow codecs, and Allow codecs.
- [ ] CORE-IAX-DEV-004 — Dial string, Accountcode, Mailbox, Deny, and Permit.
- [ ] CORE-IAX-DEV-005 — Require Call Token: yes, no, or auto.
...
- [ ] CORE-PJSIP-TRUNK-ADV-003 — Send Connected Line.
...
- [ ] CORE-PJSIP-TRUNK-ADV-011 — Contact User.
- [ ] CORE-PJSIP-TRUNK-ADV-012 — From Domain and From User.
- [ ] CORE-PJSIP-TRUNK-ADV-013 — Client URI and Server URI.
- [ ] CORE-PJSIP-TRUNK-ADV-014 — Media Address.
...
- [ ] CORE-PJSIP-TRUNK-ADV-018 — T.38 UDPTL support.
- [ ] CORE-PJSIP-TRUNK-ADV-019 — T.38 Error Correction: None, Forward, or Redundancy.
- [ ] CORE-PJSIP-TRUNK-ADV-020 — T.38 NAT and Maximum Datagram.
- [ ] CORE-PJSIP-TRUNK-ADV-021 — Fax Detect.
- [ ] CORE-PJSIP-TRUNK-ADV-022 — Trust RPID/PAI.
- [ ] CORE-PJSIP-TRUNK-ADV-023 — Send RPID/PAI: No, Remote-Party-ID, PAI, or Both.
- [ ] CORE-PJSIP-TRUNK-ADV-024 — Send Private Caller-ID Information.
...
- [ ] CORE-ADV-UI-001 — Grouped accordion/settings-list renderer with Submit, Reset, modified indicators, and per-setting Restore Default.
- [ ] CORE-ADV-UI-002 — Read-only values hidden or displayed by policy, with a deliberate and audited override flow.
- [ ] CORE-ADV-UI-003 — Hidden settings remain inaccessible unless an explicit product policy allows them; do not reproduce Core's dead hidden-setting preference.
- [ ] CORE-ADV-UI-004 — Type, option, path, range, and read-only validation before write.
- [ ] CORE-ADV-UI-005 — Backup warning, impact summary, reload/service-restart preview, readback, and rollback.
- [ ] CORE-ADV-UI-006 — UI language refresh and secret-setting validation without rendering stored secret values.
- [ ] CORE-ADV-UI-007 — Asterisk/module/version-specific visibility with a reason for every omitted setting.
...
- [ ] CORE-ADV-024 `HTTPENABLED` — Enable the Asterisk mini-HTTP server.
- [ ] CORE-ADV-025 `HTTPENABLESTATIC` — Enable mini-HTTP static content.
- [ ] CORE-ADV-026 `HTTPENABLESTATUS` — Enable the mini-HTTP status page.
- [ ] CORE-ADV-027 `HTTPBINDADDRESS` — HTTP bind address.
- [ ] CORE-ADV-028 `HTTPBINDPORT` — HTTP bind port.
- [ ] CORE-ADV-029 `HTTPPREFIX` — HTTP URL prefix.
- [ ] CORE-ADV-030 `HTTPTLSENABLE` — Enable HTTPS/TLS.
- [ ] CORE-ADV-031 `HTTPTLSBINDADDRESS` — HTTPS bind address.
- [ ] CORE-ADV-032 `HTTPTLSBINDPORT` — HTTPS bind port.
- [ ] CORE-ADV-033 `HTTPTLSCERTFILE` — TLS certificate path.
- [ ] CORE-ADV-034 `HTTPTLSPRIVATEKEY` — TLS private-key path.
- [ ] CORE-ADV-035 `TLSDISABLEV1` — Disable TLS 1.0.
- [ ] CORE-ADV-036 `TLSDISABLEV11` — Disable TLS 1.1.
- [ ] CORE-ADV-037 `TLSDISABLEV12` — Disable TLS 1.2.
- [ ] CORE-ADV-038 `HTTPSESSIONLIMIT` — HTTP session limit.
- [ ] CORE-ADV-039 `HTTPSESSIONINACTIVITY` — HTTP session inactivity timeout.
- [ ] CORE-ADV-040 `HTTPSESSIONKEEPALIVE` — HTTP session keepalive.
...
- [ ] CORE-FCODE-008 — General Call Pickup, default `*8`.
- [ ] CORE-FCODE-009 — Blind Transfer, default `##`.
- [ ] CORE-FCODE-010 — Attended Transfer, default `*2`.
- [ ] CORE-FCODE-011 — Abort Attended Transfer, default `*3`.
- [ ] CORE-FCODE-012 — Complete Attended Transfer, default `*4`.
- [ ] CORE-FCODE-013 — Complete as Three-Way, default `*5`.
- [ ] CORE-FCODE-014 — Swap Transferee and Destination, default `*6`.
- [ ] CORE-FCODE-015 — Toggle Call Recording / one-touch automon, default `*1`.
- [ ] CORE-FCODE-016 — Disconnect in-call feature mapping, default `**` where applicable.
