...
- [ ] Give the built About destination the design's own heading. Its `<h1>` reads `About Ding PBX Console` where the design's reads `About`, so the settle condition that proves a driver arrived cannot be satisfied, and it is the one destination with no built capture.
...
- [ ] **Build the seven menus in the menu bar.** File, Edit, View, PBX, Agent, Window and
...
- [ ] **Stop claiming actions that never happened.** Five notifications state an outcome
...
- [ ] **Make the two canvas actions real.** Adding a step and duplicating a node both
      report a change to the canvas that the canvas never receives.
- [ ] **Make copy-to-clipboard actually copy.** Two handlers report that a value was
