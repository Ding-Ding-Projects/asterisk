...
- [x] Mount the five independent attention modes from durable settings, add live desktop consumers, persist the user-selected next action, acknowledge serialized durable writes, persist redacted warning and error history, pass explicit notification severity, and route exact mutation actions through one generated callback without treating navigation or reads as changes. Status: implemented-unverified in `codex/mount-attention-runtime`; the design compiler ran, but no built-artifact or UI evidence was collected in that lane.
