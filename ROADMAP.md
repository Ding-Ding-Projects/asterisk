...
- [x] Add a browser-local authenticator equivalent with manual Base32 registration, URI import, bundled in-process QR rendering, live current and next TOTP codes, encrypted browser-local storage, and ordinary-export secret omission. Platform vault storage and runtime proof remain unavailable.
- [x] Add local fictional Support Tickets with no-network disclosure, canned response, local persistence, and clear-browser-storage recovery copy.
- [x] Add a partial local unlock-ladder equivalent with a bounded arithmetic rung that clears only its own wait, never signs in, sets a cookie, or refunds attempts. Server-side nonce grading and the dim-sum and whack-a-mole rungs remain unavailable.
- [x] Add narrow 320px layout rules for the new tab manager, status projection, and guided form surfaces.
- [x] Add bounded append-only browser history for site settings, tabs, appearance, locks, tickets, ladder, and status metadata, with stable IDs, date/action filters, diff, restore records, bulk selection, and redacted export. Runtime proof remains unverified.
