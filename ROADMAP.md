...
- [ ] Mount and verify the desktop forge-publishing runtime: GitHub account discovery, active-account confirmation, valid stdin-confirmed sign-out, direct device sign-in with surfaced user code and URL plus bounded polling and stdin-only credential installation, inherited-auth clearing, provider-supplied vault references only, provider-backed owner selection with partial capabilities, exact fork and copy-and-push route semantics, bounded `shell:false` `gh`/`git` execution with common settlement, destination identity and effective push-URL `ls-remote` proof, atomic versioned state with visible corruption, separate account and publication receipts that survive cancellation and reload, local history, cancellation, progress, hosted refusal, regex-builder binding, and beside-control re-authentication are implemented on the feature branch; runtime provider verification and built-artifact evidence remain pending.
