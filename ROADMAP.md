...
- [x] **External editor handoff** — mounted desktop detection for Visual Studio Code stable, Insiders and bounded Portable discovery plus arbitrary product-metadata-verified portable paths, bounded custom editor records with explicit folder capability, native file and folder picking, privileged-only choice persistence with invalid-store reset, exact PBX read-back materialization into local exports, operation ids with bounded progress and cancellable child or temporary-file cleanup, shell-free typed launch receipts, file versus folder checks, exact per-editor download allowlists, a complete producer inventory, and honest hosted no-editor behavior.
