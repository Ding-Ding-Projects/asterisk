...
- [ ] Add reference-versus-built captures and visual diffs, so destinations can move from `compiled` to `verified`. **The captures and diffs now exist and are committed** — 32 reference captures of the real design export rendered by its own runtime, 31 built captures of the real built renderer under Electron, and 31 visual diffs with side-by-side comparisons, all at the same `default/dark/1440x1000/1x` tuple, taken on an off-screen Windows desktop with zero requests reaching the network. Two things the previous pass recorded as blockers turned out not to be blockers at all: `design/support.js` loads React itself and reads `window.__resources` first, so the runtime is now vendored locally and verified against the design's own integrity hashes without editing anything under `design/`; and the built side needs no `ding-pbx://` protocol handler, only the same rail-then-section clicks the reference side performs. **No row moved to `verified`**, because that needs a `match` verdict and `compareCaptures` means pixel-identical — a bar this project made unreachable on purpose when it removed the design's sample rows and tiles from the running application. The 31 diffs measure exactly that: 47%–64% of pixels differ, none refused as unpainted or stale, identical chrome beside different data. What remains is the three items below. **[Deliberately still unticked, and here is the reading order, because an unattended pass keeps landing on this line first and it is not the workable one.** The captures-and-diffs half of this item is finished; the `compiled` → `verified` half depends entirely on the two items below it, which are its real content. Item 22 is now done, so nothing about the About heading blocks a capture any more. Item 20 is now done too, so a destination has a bar to be measured against and the reading order has moved on. **Take item 21 next (the Material Design 3 audit)** — it is the last of the three, and it is the one remaining prerequisite the `verified` guard names. The two pieces of *this* item that are left are both consequences of what item 20's first run found, and neither is bookkeeping: take About's 32nd chrome record from a build newer than the About-heading repair, and decide where the shell-geometry divergence gets repaired — in the application, in the design, or in the capture harness — since that single cause is what currently keeps all 31 records on `diff`.**
...
- [ ] Perform the per-destination Material Design 3 conformance audit the `verified` guard also requires. No `materialAudit` file was generated, deliberately: a machine-written `conforms: true` that nobody performed would be an invented verdict.
