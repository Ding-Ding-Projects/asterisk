...
- [ ] Mount and verify the desktop forge-publishing runtime: GitHub account discovery, active-account confirmation, keyring proof with plaintext fallback refusal, valid stdin-confirmed sign-out with independent reread, client-id-free ConPTY device sign-in with surfaced user code and URL plus per-request deadlines, bounded polling and stdin-only credential installation, inherited-auth clearing, provider-supplied vault references only, provider-backed owner selection with unknown organization capabilities until proven, exact fork and copy-and-push route semantics, bounded `shell:false` `gh`/`git` execution with common settlement, destination identity and exactly-one effective push-URL `ls-remote` proof, atomic versioned state with visible corruption, separate action-specific receipts that survive cancellation and unknown side effects, local history, cancellation, progress polling by operation id, hosted refusal and disabled controls, regex-builder binding to forge account data, pinned gh and ConPTY helper package verification, and beside-control re-authentication are implemented on the feature branch; the direct HTTP route still needs an immutable approved public OAuth client id, while runtime provider verification and built-artifact evidence remain pending.
...
- [x] **Language modes** — the compiled renderer now applies English, Hong Kong Cantonese, or bilingual copy to visible and accessible text, with durable selection and English fallback for missing translations.
- [x] **Funny-level sliders** — independent English and Cantonese levels 1 to 5, defaulting to 5, persisted separately and wired into notification and dialog copy without changing facts.
- [x] **School mode** — shared durable state with live refresh, renamable label, encrypted desktop credential unlock, forced English, and restoration of the previous language and funny levels.
- [x] **Dialog emoji switch** — persisted opt-in decoration remains available outside School mode and never reaches controls or accessible names.
