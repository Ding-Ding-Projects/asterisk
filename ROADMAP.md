...
- [ ] Mount the Status Hub and browser-extension transfer clients. The implementation is on `codex/mount-status-downloads`, but tests, build, runtime interaction, extension handoff, and captures remain pending. Status Hub availability depends on `STATUS_HUB_URL`; transfer state is persisted in `download-transfers.json`.
