...
- [ ] Decide whether the compiler should stop pinning the Material Symbols variation axes. `compile-design.mjs` adds `font-variation-settings:"FILL" 0,"wght" 400,"GRAD" 0,"opsz" 24` to its `.msym` rule; the design declares no such property anywhere, so the built side draws every icon in the application from a pinned instance of the variable font and the reference side draws it from the file's default one. That is the measured cause of most of `statusCell`'s 9.65%, and it will be a component of `brandCell`, `menuCell`, `tabStrip`, `rail` and `sectionList` too, since every one of them contains icons. It is **not** obviously a defect: the pinning arrived with the 49-face font download precisely because the earlier package substitute "gave Material Symbols a face whose variation axes the design's `.msym` rule could not actually drive", so removing it may be undoing a repair. Whoever takes this should first *demonstrate* the cause rather than inherit it as named — render one destination both ways and confirm the icon pixels converge — and note that changing it alters how every icon in the shipped product is drawn and invalidates all 32 built captures, so it needs a full two-sided capture run behind it, not a role change.
- [ ] Find where the mode picker's half pixel enters. On every one of the 32, the built side draws the 28px-tall Beginner/Expert picker's 1px border split across two rows at `rgb(24,31,25)` and `rgb(40,52,45)` where the reference draws it crisp on one row at `rgb(65,73,66)`, and the same at the bottom edge — 548 of the 946 pixels differing inside that control. Both sides report a 40px top strip and a 28px picker, and `(40 − 28) / 2` is an integer, so the offset has no obvious source. It is not a whole-frame shift: the byte-identical Roboto digit two cells along proves at least one glyph sits at exactly the same subpixel position on both sides. Worth doing because it is the only measured divergence in this application with **no** identified cause at all, and because a half-pixel box offset will not be confined to this one control.
