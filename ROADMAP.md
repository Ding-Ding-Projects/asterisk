...
- [ ] Find where the mode picker's half pixel enters. On every one of the 32, the built side draws the 28px-tall Beginner/Expert picker's 1px border split across two rows at `rgb(24,31,25)` and `rgb(40,52,45)` where the reference draws it crisp on one row at `rgb(65,73,66)`, and the same at the bottom edge — 548 of the 946 pixels differing inside that control. Both sides report a 40px top strip and a 28px picker, and `(40 − 28) / 2` is an integer, so the offset has no obvious source. It is not a whole-frame shift: the byte-identical Roboto digit two cells along proves at least one glyph sits at exactly the same subpixel position on both sides. Worth doing because it is the only measured divergence in this application with **no** identified cause at all, and because a half-pixel box offset will not be confined to this one control. **Left unticked, but the symptom it describes is gone and the premise needs restating before anyone spends time on it.** The axis-pin pass above retook both sides under one browser, where the reference had been running under headless Edge and the built side under Electron. In the new captures **not one pixel in the picker's border rows differs**: `statusCell`'s remaining 555 differing pixels, identical on all 32, are entirely in the text band, rows 14–25, in three column runs matching the check glyph and the two labels. So the half pixel was a property of comparing two browsers rather than of either artifact, and the reason `(40 − 28) / 2` looked like it had nowhere to come from is that it never came from the layout. What is left to explain is not a box offset but those 555 text pixels, which is a different question and a smaller one. Nobody has looked at it.
