...
- [x] **Migration, verified backups, and local Git management** — versioned non-secret manifest, machine-readable omissions, verified `git bundle` history, staged import with backup and rollback, bounded backup records, normal non-force remote receipts, and a frozen short-lived prune preview token required before candidate enumeration or deletion. A blank push URL clears a custom push URL and falls back to the validated fetch URL after readback. Runtime import and remote mutation evidence remains open.
