...
- [ ] Decide whether `statusCell` stops being a data area of the chrome-parity bar. Found while doing the item above, and it is the same defect one cell along: its declared reason reads "carries live status: what the console is connected to and how that connection is faring", and that sentence describes `commandCell`, not this cell. The fourth top cell is the Beginner/Expert mode picker, the confirmation-credits pill, the command-palette button and the three window controls — local application state and fixed chrome, with no target reading in it at all. On that description the cell is chrome, and excluding it is a narrowing nobody argued for. The declaration has been corrected to say what the cell actually renders; the **role has deliberately not been changed**, for the reason `commandCell`'s was not changed in the pass that corrected `commandCell`'s text: two areas moving at once leaves one compared fraction nobody can attribute to either move. Including it would raise the compared fraction from 28.0883% to roughly 29.11%. Whoever takes this should change one thing and re-run `--side=chrome` alone — and note that it needs the build output the captures were taken from, not a fresh build, or every record comes back `refused` on a staleness check comparing checkout times.
