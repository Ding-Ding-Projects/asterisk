...
- [x] Add immutable public catalog photo records with actual consumers, strict versioned settings parsing, timezone-aware base/effective schedule layers, boundary refresh, approved external host policy, reduced Home Assistant proxy validation, shared School filtering, exact palette controls, anchored regex metadata, and super-confirmed schedule and display-name removal.
- [x] Resolve the fourth settings refutation: generated article toolbar and palette fallback markup, one canonical versioned state with a compatibility mirror, exact five-level funny synchronization, verified local dim-sum cache, deadline-bounded fetches, identity cache purge, exact schedule boundary timing, invalid-timezone visibility, explicit source policy, bilingual segments, global narrator event bridging, and physically anchored regex ownership.
- [ ] Keep the dim-sum feature partial until first-use public catalog fetch and local image caching have succeeded on the target browser.
- [x] Resolve the fifth settings refutation: early keyboard shortcut registration, generated palette base correction, neutral bilingual event payloads, one canonical state with owned mirror fields, catalog-v1-only dim-sum authority, shuffled bounded image bootstrap, full cache identity and purge, exact schedule transitions, target validation, and Home Assistant host policy.
- [x] Resolve the sixth settings refutation: five distinct copy levels, School-name-derived labels, complete copy inventory, source-track events, exact catalog-v1 identity, stored-rule validation, signature-based effective transitions, physical regex collision geometry, and truthful partial registry state.
