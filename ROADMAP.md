...
- [x] **Accessibility, overlays, and recovery consumers** — shared focus-visible and target-size rules, persisted attention root consumers, self-painted bounded overlays with keyboard movement and geometry reset, searchable context menus and dropdowns with plain-text-first anchored regex builders and shortcut labels, generation-checked cancellable progress with a semantic settled progressbar, in-context retry and re-authentication callbacks, durable notification history with versioned export and confirmation-protected bulk dismissal, and collapsible filters that retain active-state warnings. Compiled from the checked-in design source on 2026-08-24; no test, lint, broad build, desktop interaction, or HuiShot evidence was run in this lane.
