...
- [x] **Accessibility, overlays, and recovery consumers** — shared focus-visible and target-size rules, reduced-motion support, self-painted bounded overlays, searchable context menus and dropdowns with anchored regex builders and shortcut labels, cancellable one-click progress with re-entry protection, in-context retry and re-authentication actions, runtime notification history with bulk actions, and collapsible filters that retain active-state warnings. Compiled from the checked-in design source on 2026-08-24; no test, lint, broad build, desktop interaction, or HuiShot evidence was run in this lane.
