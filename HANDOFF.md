...
- `console/site/app.js` and `console/site/settings.html` now include partial browser-local authenticator, fictional Support Tickets, and partial unlock-ladder equivalents. Authenticator secrets are excluded from ordinary export. QR rendering and platform vault storage remain unavailable. The ladder only clears its own local wait and never creates a session, cookie, or refunded attempt.
