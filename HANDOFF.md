...
The latest repair queues pre-bootstrap universal-state patches, flushes them through `DurableStorageHandle`, and exposes durable-store refusal in the toolbar status. The snapshot is deep-validated fail-closed, including schema version, bounded record counts, notification ids and sources, known tab and pin ids, unique grouped-tab ownership, colours, collapsed and hidden state, geometry keys and finite bounds, and allowed docking values before applying it. Cancellation uses the actual bridge request id and AbortSignal when the bridge exposes cancellation, while preserving the remaining non-cancellable boundary honestly. Retry consumes the stored allowlisted failed-action descriptor and requests its exact recorded action plus view or resource, never substituting a generic screen read. Group and tab menus expose the same recovery route. Every regex field has an isolated `{query, pattern, flags, sample}` tuple, match count, capture, copy, and export state. This lane remains implemented-unverified until runtime interaction evidence exists.
