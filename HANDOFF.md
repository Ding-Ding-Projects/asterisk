...
The browser-extension handoff enters through `download:submit-handoff`, is bounded by `isExtensionDownloadHandoff`, and is persisted with transfer snapshots in the installation data file `download-transfers.json`. The native picker or a persisted approved root supplies the destination, and the manager rejects mismatched handoff replay, non-absolute paths, containment escapes, and symlink or reparse components immediately before writing. It streams HTTPS bytes into a unique adjacent temporary file, applies header, body-idle, and total deadlines, validates bytes, and publishes through the shared Windows rename retry helper. Range pause and resume use recorded validators when supported, while unsupported controls stay disabled with an exact reason. Startup reconciliation turns interrupted work into partial or failed state. Dedicated always-on-top BrowserWindows host Start, Downloading, and Complete, and terminal failed or cancelled states reach the final outcome window. The preload exposes start, cancel, command, snapshot, subscription, and handoff listeners. The companion extension is not part of this Oak Kay, so no extension source or fake handoff was added.
