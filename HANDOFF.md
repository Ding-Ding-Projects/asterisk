...
- `deployer/deployment/preflight.ps1` is a read-only inventory for WSL, local Docker, and an explicitly approved private host. Host mode requires an exact private-inventory tuple and persistent protected known-hosts path, then reports actual architecture, CPU, memory, free bytes, port conflicts, SSH exit state, Docker state, and each workload without starting, stopping, pruning, importing, removing, or deploying anything.
- `deployer/deployment/deploy-control-plane.ps1` requires an external immutable deployment manifest and matching timestamped preflight evidence, reads embedded provenance for the source commit and version, verifies TLS paths, writes a pre-change snapshot of the five persistent volumes, waits for local CLI readiness and authenticated server readiness, verifies owned container, digest, volume, tmpfs, and network state, and automatically restores the previous image when the new image fails. Plan-only mode performs no pull, create, or remove action. `deployer/deployment/rollback.ps1` binds the previous Pow Gook to its own manifest and retains every persistent volume.
