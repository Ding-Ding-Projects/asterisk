...
- Mounted the desktop forge-publishing runtime on the History screen. The typed bridge now discovers local GitHub CLI accounts, stores only account ids and OS-vault references, supports active-account, refresh, add, and sign-out actions, reads personal and organization owners from provider responses, distinguishes GitHub and unavailable GitLab capabilities, and offers separate fork and copy-and-push routes. `gh` and `git` calls use the allowlisted `shell:false` executor with deadlines and redacted output. Account mutations and publication receipts enter local append-only history, while failed or missing sign-in remains an explicit re-authentication state. The feature is partial until runtime provider verification and built-artifact evidence land.
