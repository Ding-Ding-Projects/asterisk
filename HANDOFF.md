...
- `deployer/deployment/deploy-control-plane.ps1` requires an external immutable deployment manifest and matching preflight evidence, reads embedded provenance for the source commit and version, verifies TLS paths, waits for liveness and local Asterisk readiness, verifies owned container, digest, volume, and network state, and automatically restores the previous image when the new image fails. Plan-only mode performs no pull, create, or remove action. `deployer/deployment/rollback.ps1` prints a plan by default and retains every persistent volume.
- `console/server/http-server.ts` exposes a fixed redacted `/api/v1/health` liveness route and an authenticated `/api/v1/ready` target-readiness route. First setup refuses non-loopback HTTP and uses a single-use nonce plus an atomic account write. Request, header, socket, and body limits are bounded.
