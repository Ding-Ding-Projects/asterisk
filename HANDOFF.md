...
- Mounted the desktop forge-publishing runtime on the History screen. The typed bridge now discovers local GitHub CLI accounts, confirms the active login for each owner read, clears inherited authentication variables, stores only account ids and provider-supplied vault references in atomic versioned state, supports direct device sign-in with surfaced user code and verification URL plus bounded polling and stdin-only credential installation, valid stdin-confirmed sign-out, refresh, add, and cancellation, reads personal and organization owners with partial permissions, distinguishes GitHub and unavailable GitLab capabilities, and offers exact fork and copy-and-push routes with destination identity and effective push-URL `git ls-remote` proof. `gh` and `git` calls use the allowlisted `shell:false` executor with deadlines, redaction, and common settlement. Account and publication receipts are separate, reloadable, and retain failed, partial, cancelled, and unknown-side-effect outcomes. Hosted refusal, visible corruption reset, regex binding, and progress terminal states are explicit. The feature is partial until runtime provider verification and built-artifact evidence land.
