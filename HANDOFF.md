...
The complete-surface lane added a deterministic source catalogue at `console/control-plane/generated/asterisk-catalog.json` and `asterisk-catalog.ts`. It currently contains **387 source modules, 119 configuration resources, and 11 ARI resource documents**, for **517 records**, generated from the 12 Asterisk source families, checked-in `configs/` resources, and `rest-api/api-docs/`. The generator is `console/scripts/generate-asterisk-catalog.mjs`, strips comments and string literals before token-aware registration parsing, records SHA-256 source and build-graph provenance, and uses a fixed timestamp for byte-stable output.

`console/control-plane/asterisk-runtime-catalog.ts` reconciles those records with target observations. The `pbx.catalog` action reads the real `module show`, `core show help`, `manager show commands`, `ari show apps`, and target configuration filename inventory through bounded allowlisted executable-argument paths. Unread or incomplete surfaces remain `unknown`, absent modules are `unavailable`, observed modules are `available`, and newly installed modules that are not in source remain visible as `unverified-installed-module` records with a read-only action boundary. `AmiTransport` and `AriTransport` provide named, bounded operations with OS-vault credential lookup, cancellation, timeouts, response limits, and redacted typed receipts. No source-only run claims runtime verification.

The generated `ami-ari-registry.ts` carries all **122 AMI actions, 11 AMI events, and 109 ARI operations** as stable typed records. `pbx.catalog` keeps AMI and ARI family state independent: missing AMI credentials mark AMI operations unavailable while CLI observations can remain available, and missing ARI credentials mark ARI operations unavailable without changing module or configuration state. Mutating AMI and ARI operations require explicit confirmation. The module and CLI surfaces use a current catalogue revision, live record checks, single-use module confirmation, and post-action rereads.
