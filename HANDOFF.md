...
The first repair deliberately left conversion unavailable because no bundled isolated decoder existed at that point. The follow-up now registers the packaged worker and keeps the previous active mark after any decoder or cache poke guy. The Home Assistant path carries only `vaultAccountKey`; the desktop dispatcher injects an OS-backed `safeStorage` reader, while missing or undecryptable references remain typed unavailable. The settings route is mounted and hydrated, but no tests, lint, type checks, build, package, UI interaction, capture, or release work was run in this lane. The feature registries and three platform articles record these exact boundaries.
...
Logo cache writes now reject unknown fields and duplicate manifest keys, bound manifest bytes, validate complete targets, receipts, crop, package identity, digests, exact byte counts, output count, and aggregate output bytes before replacing the active cache. A decoder interface reopens every output through its isolated boundary. No checks, builds, packaging, UI runs, captures, pushes, or cleanup were performed.

### Decoder and application-lifetime repairs

The follow-up repair adds the declared `sharp` dependency and lockfile entries, a packaged worker resource, and an isolated process decoder with bounded input pixels, static-frame enforcement, crop and target bounds, deadline, memory receipt, output bytes, aggregate output pixels, and independent reopen validation. Renderer cache writes are no longer accepted; successful privileged conversion writes the cache internally after reopen validation. The application-lifetime runtime now hydrates once from `main.tsx`, applies effective language, theme, density, display name, narration, attention, appearance, and logo state to the document, and the settings route reuses that singleton. Home Assistant credentials can be enrolled through the desktop vault picker and are stored only through `safeStorage`. No checks, builds, packaging, UI runs, captures, pushes, or cleanup were performed.

### Fourth refutation repairs

The decoder now requires a bounded child health handshake that imports `sharp`, fully decodes and reopens one-pixel outputs in PNG, JPEG, and WebP, and reports worker revision, runtime version, native version, capabilities, working-set evidence, and locked-package integrity. Windows launches the child under a packaged Job Object helper with a native process-memory ceiling, while the parent queries actual working set and terminates an over-limit child. The worker receives a minimal environment and refuses to run without its no-network bootstrap assertion.
