...
`console/catalog/freepbx-module-catalog.json`. It was generated by
`console/scripts/generate-freepbx-module-catalog.mjs` using the official `gh api` route against
the public `FreePBX` organization and each repository's `release/17.0/module.xml`. The catalog
contains 83 public module entries, zero local module entries because no local module directory was
supplied during generation, and 83 entries marked unavailable or unverified. Each entry records its
module ID, name, version, repository and module metadata revisions, license and entitlement class,
dependencies, configuration resources, fwconsole command metadata, API capabilities, UI families,
documentation links, local installation state, and exact availability reason. The catalog includes
a SHA-256 snapshot over the pinned module and metadata revisions.

`console/app/renderer/src/freepbx-module-catalog.ts` validates the catalog at load time and provides
family search. `console/app/renderer/src/freepbx-module-form.ts` builds native schema-driven fields
from values actually read from the target, uses the existing sample-backed closed-enumeration
catalog, and creates structured module action requests. `console/inventories/freepbx-module-surface.json`
is the hand-written per-module and family inventory. `console/scripts/verify-freepbx-module-inventory.mjs`
checks exact catalog/inventory equality and its `--probe-negative` mode demonstrated red on a removed
module and green after restoration.

Documentation is in `console/docs/pbx/freepbx-modules.md`, indexed by `console/docs/pbx/README.md`.
This lane did not run tests, lint, type checks, full build, package, UI interaction, browser capture,
