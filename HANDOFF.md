...
- `deployer/deployment/deploy-control-plane.ps1` requires an external immutable deployment manifest and matching timestamped preflight evidence, reads embedded provenance for the source commit and version, verifies TLS paths and a pinned certificate, enforces a fresh protected short-lived readiness credential, writes a durable pre-change snapshot journal for all five persistent volumes, proves archive recovery before Compose mutation, waits for host reachability, local CLI readiness, and authenticated server readiness, verifies owned container, digest, volume, tmpfs, and network state, and automatically restores the compatible previous image and volume state when the new image fails. Plan-only mode performs no pull, create, or remove action. `deployer/deployment/rollback.ps1` binds the previous image to its own manifest and blocks incompatible volume schemas.
