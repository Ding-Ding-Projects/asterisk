...
- `console/site/app.js`, `console/site/settings.html`, and bundled `console/site/qr-encoder.js` now include a browser-local authenticator equivalent, fictional Support Tickets, and partial unlock-ladder equivalents. Authenticator registration supports URI import, explicit reveal, matching QR output, encrypted browser-local storage, and ordinary-export secret omission. Platform vault storage remains unavailable. The ladder only clears its own local wait and never creates a session, cookie, or refunded attempt.
- The Status Hub projection validates receipt shape and matching commit, tag, evidence URL, and evidence hash, but this static surface has no trusted signature verifier. Pasted public values therefore remain `User-declared, unverified`; a Setup URL alone is never sufficient.
- Writable converter output checks for an existing destination file and opens the site's two-key, full-slider overwrite confirmation before replacement. New files write directly after the user-selected folder permission is granted.
