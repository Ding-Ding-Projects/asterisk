...
The latest repair queues pre-bootstrap universal-state patches, flushes them through `DurableStorageHandle`, and exposes durable-store refusal in the toolbar status. The snapshot validates version, record count, known tab ids, group membership, colours, collapsed and hidden state, geometry bounds, and notification states before applying it. Cancellation races the actual bridge promise and records the remaining non-cancellable boundary honestly. Retry consumes the stored failed-action descriptor and requests its exact screen reading, while group and tab menus now expose the same recovery route. Every regex field has isolated sample, match, capture, copy, and export state.
