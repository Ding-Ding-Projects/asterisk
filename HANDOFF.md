...
The repair lane keeps those routes desktop-only when the browser bridge cannot provide a credential vault. Stored authenticator secrets no longer cross back to the renderer. Current and next codes are generated in the privileged boundary. Toy-lock passwords use salted scrypt records, TOTP values remain encrypted and separately recoverable, and vault writes use the shared bounded Windows rename helper. Unlock-ladder mole hits use privileged receipts, School mode is read from the shared settings record, and a ladder clear must remove an authoritative wait before reporting success. Authenticator and lock removal now use the native two-key plus full-range slider confirmation surface. This repair lane remains unverified because no Chuts, builds, UI runs, or captures ran.

The latest auth-lock mount now injects one durable `NotificationStore` into the visible authenticator surface, exposes factual authenticator and lock reconciliation receipts, preserves pending and unresolved removal identifiers, and blocks contradictory mutations until reconciliation settles. Lock outcomes and history restore outcomes remain typed through dispatch, bridge, and renderer. The feature registry remains `implemented-unverified` until built proof exists.

The checked-in generated documentation bundle still needs regeneration from the updated source articles in the next integration lane. No generated bundle file was edited here.
