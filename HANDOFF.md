...
- The shared export engine now includes a validated store-mode ZIP writer and an explicit archive catalog. Its independent reopen pass rejects unsafe or colliding archive entry names as well as structural corruption. 7z options are listed but disabled until a bundled adapter exists. The History screen has JSON, ZIP, batch restore, and external-editor handoff seam actions. Restore resolves only commits in the bounded parsed history listing before it reads a tree, and appends its own restore evidence.
