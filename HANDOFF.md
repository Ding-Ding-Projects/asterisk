...
- The follow-up recovery repair adds encrypted snapshots with versioned HKDF-derived keys, outer-finally cleanup, owner and reparse-point checks, standalone restore TLS and readiness validation, complete plan-only commands, image ID and RepoDigests evidence, source and input hash matching, bounded whole-operation deadlines, and verified retention cleanup. Registry publication remains implemented but unverified.
