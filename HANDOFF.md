...
The latest repair queues pre-bootstrap universal-state patches, flushes them through `DurableStorageHandle`, and exposes durable-store refusal in the toolbar status. The snapshot is deep-validated fail-closed, including schema version migration, bounded record counts, notification ids and sources, known tab and pin ids, unique grouped-tab ownership, colours, collapsed and hidden state, separate dialog-size `{w,h}` bounds, geometry keys and finite bounds, allowed docking values, and a UTF-8 byte cap before applying it. Cancellation is explicitly renderer invalidation only because the shipped bridge has no cancellation IPC, and the remaining pending boundary is surfaced honestly. Actual failed control-plane actions record their exact descriptor; context menus show Refresh unless a real descriptor exists. Retry consumes the stored allowlisted descriptor and requests its exact recorded action plus view or resource, never substituting a generic screen read. Group and tab menus expose the same recovery route. Every regex field has an isolated `{query, pattern, flags, sample}` tuple, with raw and guided mutations routed through `setRegexField`. This lane remains implemented-unverified until runtime interaction evidence exists.
