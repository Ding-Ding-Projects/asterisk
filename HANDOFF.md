...
- `deployer/deployment/docker-compose.yml` is the hosted runtime contract. It uses a read-only root filesystem, dropped capabilities, `no-new-privileges`, bounded temporary filesystems, memory, CPU, PID, file-descriptor, and process limits, `restart: unless-stopped`, named volume `ding-pbx-control-plane-data`, an internal egress-restricted network, and Docker-mounted TLS secrets. It does not place secret bytes in the image or environment.
- `deployer/deployment/preflight.ps1` is a read-only inventory for WSL, local Docker, and an explicitly approved private host. Host mode requires an exact private-inventory tuple and persistent known-hosts path, then reports architecture, CPU, memory, disk capacity, port conflicts, Docker state, and existing workloads without starting, stopping, pruning, importing, removing, or deploying anything.
- `deployer/deployment/deploy-control-plane.ps1` validates immutable `image@sha256` references, reads embedded provenance for the source commit and version, verifies TLS paths, waits for liveness, and automatically restores the previous image when the new image fails. `deployer/deployment/rollback.ps1` prints a plan by default and retains the persistent volume.
...
- `console/control-plane/dispatch.ts` supports an explicit hosted local Asterisk transport, refuses missing or unknown hosted transport by name, refuses hosted WSL and daemon lifecycle actions by name, and hashes the packaged WSL rootfs in bounded chunks against its trusted manifest, source commit, base digest, byte count, SHA-256, and 8 GiB upper bound.
