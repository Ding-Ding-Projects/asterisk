...
- Mounted the desktop forge-publishing runtime on the History screen. The typed bridge now discovers local GitHub CLI accounts, confirms the active login, clears inherited authentication variables, stores only account ids and OS-vault references in atomic versioned state, supports device sign-in, noninteractive sign-out, refresh, add, and cancellation, reads personal and organization owners with partial permissions, distinguishes GitHub and unavailable GitLab capabilities, and offers exact fork and copy-and-push routes with `git ls-remote` destination proof. `gh` and `git` calls use the allowlisted `shell:false` executor with deadlines and redacted output. Account mutations and publication receipts enter local append-only history and reload after restart, while failed, partial, cancelled, hosted, or missing sign-in states remain explicit. The feature is partial until runtime provider verification and built-artifact evidence land.
