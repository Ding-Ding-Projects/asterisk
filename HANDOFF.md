...
- `deployer/deployment/control-plane.Dockerfile` builds Asterisk and the hosted console from the exact checkout commit supplied by `build-control-plane.ps1`. Ubuntu and Node build bases are digest pinned, Ubuntu packages use the committed snapshot timestamp, Node `22.23.2` is copied from the pinned build stage, and apt plus runtime input records are written into the image. The runtime has a dedicated non-root UID/GID, explicit port `8088`, a redacted liveness healthcheck, OCI source and version labels, source-tree, Dockerfile, lockfile, and input-manifest digests, a persistent data volume, and `/opt/ding-pbx-console/provenance.json`.
- `deployer/deployment/docker-compose.yml` is the hosted runtime contract. It uses a read-only root filesystem, dropped capabilities, `no-new-privileges`, bounded temporary filesystems, memory, CPU, PID, file-descriptor, and process limits, `restart: unless-stopped`, five owned writable persistent volumes plus an owned `/run/asterisk` tmpfs, an admin-only internal network with no PBX ports published, and Docker-mounted TLS secrets. It does not place secret bytes in the image or environment.
- `deployer/deployment/preflight.ps1` is a read-only inventory for WSL, local Docker, and an explicitly approved private host. Host mode requires an exact private-inventory tuple and persistent protected known-hosts path, then reports actual architecture, CPU, memory, Docker engine storage facts, port conflicts, SSH exit state, Docker state, and each workload without starting, stopping, pruning, importing, removing, or deploying anything. Workload conflicts are parsed from structured container records.
- `deployer/deployment/deploy-control-plane.ps1` requires an external immutable deployment manifest and matching timestamped preflight evidence, reads embedded provenance for the source commit and version without trusting a self-referential final image digest, verifies TLS paths and a pinned certificate, uses the server-issued session expiry for readiness credentials, quiesces the owned service before snapshotting, encrypts every volume archive with an operator key and HMAC integrity, writes durable snapshot and recovery journals with partial-volume outcomes, proves archive recovery before Compose mutation, waits for host reachability, local CLI readiness, and authenticated server readiness, verifies owned container, digest, volume, tmpfs, and network state, and automatically restores the compatible previous image and volume state when the new image fails. Plan-only mode performs no pull, create, or remove action. `deployer/deployment/rollback.ps1` binds the previous image to its own manifest and blocks incompatible volume schemas. Standalone restore requires fresh manifest-bound preflight, a stopped owned project, exact volume labels, the protected encryption key, and restart verification.
- `console/server/http-server.ts` exposes a fixed redacted `/api/v1/health` liveness route and an authenticated `/api/v1/ready` target-readiness route. First setup is loopback-only with no public nonce or enrollment value and uses an atomic account write. Request, header, socket, and body limits are bounded.
- `console/control-plane/dispatch.ts` supports an explicit hosted local Asterisk transport, exposes that target through discovery, refuses missing or unknown hosted transport by name, refuses hosted WSL, daemon lifecycle, configuration, media, history, connect, and snapshot actions by name, and hashes the packaged WSL rootfs in bounded chunks against its adjacent trusted manifest, an installer binding, source commit, base digest, byte count, SHA-256, and 8 GiB upper bound. `build-asterisk-wsl-bundle.ps1` writes the trusted and release manifests, while the installer package and release identity carry the externally checked release-manifest digest.
- The follow-up recovery repair adds encrypted snapshots with versioned HKDF-derived keys, outer-finally cleanup, owner and reparse-point checks, standalone restore TLS and readiness validation, complete plan-only commands, image ID and RepoDigests evidence, source and input hash matching, bounded whole-operation deadlines, and verified retention cleanup. After a standalone restore restarts Compose, it resolves the returned container ID again and rejects any stale ID, wrong project or service labels, non-manifest immutable image, unexpected ordered volume set, or missing required volume ownership label before health and authenticated readiness are accepted. Registry publication remains implemented but unverified.
