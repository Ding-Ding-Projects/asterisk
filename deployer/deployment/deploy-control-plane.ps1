...
function Snapshot-Volumes {
    ...
    $volumeLabels = @('control-plane-data', 'asterisk-etc', 'asterisk-lib', 'asterisk-log', 'asterisk-spool')
    $persistentVolumeInventory = @(); for ($index = 0; $index -lt $volumes.Count; $index++) { $inspect = @(& docker volume inspect $volumes[$index] 2>$null | ConvertFrom-Json); if ($LASTEXITCODE -ne 0 -or $inspect.Count -ne 1 -or [string]$inspect[0].Name -ne $volumes[$index] -or [string]$inspect[0].Labels.'io.ding.pbx.project' -ne 'ding-pbx' -or [string]$inspect[0].Labels.'io.ding.pbx.volume' -ne $volumeLabels[$index]) { throw "Snapshot volume $($volumes[$index]) is not the exact labeled persistent deployment volume." }; $persistentVolumeInventory += [pscustomobject][ordered]@{ volume = $volumes[$index]; projectLabel = 'ding-pbx'; volumeLabel = $volumeLabels[$index] } }
...
function Test-SnapshotRecoverability {
    ...
    if ($record.schemaVersion -ne 1 -or $record.snapshotKeyId -ne $manifest.snapshotKeyId -or $record.volumeSchemaVersion -ne $manifest.volumeSchemaVersion -or @($record.volumes).Count -ne 5 -or @($record.archives).Count -ne 5 -or -not $inventoryValid) { throw 'Snapshot recoverability is blocked by an incomplete or incompatible record.' }
